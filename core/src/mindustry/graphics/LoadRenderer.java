...
public class LoadRenderer implements Disposable {
...

    {
        ...
        bars =
                new Bar[]{
                        ...
                        new Bar(
                                "g_vtype",
                                graphics.getGLVersion().type == GlType.GLES ? 0.5f : 1f,
                                graphics.getGLVersion().type == GlType.GLES),
                        new Bar(
                                "s_mem#",
                                () -> true,
                                () -> Core.app.getJavaHeap() / 1024f / 1024f / 200f,
                                () -> Core.app.getJavaHeap() > 1024 * 1024 * 110),
                        new Bar(
                                "v_ver#",
                                () -> Version.build != 0,
                                () -> Version.build == -1 ? 0.3f : (Version.build - 103f) / 10f,
                                () -> !Version.modifier.equals("release")),
                        new Bar(
                                "s_osv",
                                OS.isWindows ? 0.35f : OS.isLinux ? 0.9f : OS.isMac ? 0.5f : 0.2f,
                                OS.isMac),
                        new Bar(
                                "v_worlds#",
                                () -> Vars.control != null && Vars.control.saves != null,
                                () -> Vars.control.saves.getSaveSlots().size / 30f,
                                () -> Vars.control.saves.getSaveSlots().size > 30),
                        new Bar(
                                "c_datas#",
                                () -> settings.keySize() > 0,
                                () -> settings.keySize() / 50f,
                                () -> settings.keySize() > 20),
                        new Bar(
                                "v_alterc",
                                () -> Vars.mods != null,
                                () -> (Vars.mods.list().size + 1) / 6f,
                                () -> Vars.mods.list().size > 0),
                        new Bar(
                                "g_vcomp#",
                                (graphics.getGLVersion().majorVersion
                                        + graphics.getGLVersion().minorVersion / 10f)
                                        / 4.6f,
                                !graphics.getGLVersion().atLeast(3, 2)),
                };
    }
    ...
    public void draw() {
        ...
        if (assets.getLoadedAssets() != lastLength) {
            ...
            for (String name : assets.getAssetNames()) {
                ...
                assetText
                        ...
                        .append(name.replace(OS.username, "<<host>>").replace("/", "::"))
                        .append(red)
                        .append("::[]")
                        .append(assets.getAssetType(name).getSimpleName())
                        .append("\n");
            }
                ...
                for (int sy : Mathf.signs) {
                    ...
                    if (panei == 0) {
                        ...
                        font.draw(
                                assetText,
                                minx + pad,
                                maxy - pad + Math.max(0, layout.height - (maxy - miny)));
                    } else if (panei == 1) {
                        ...
                        int barsUsed =
                                Math.min(
                                        (int) ((height - barpad) / (font.getLineHeight() * 1.4f)),
                                        bars.length);
                            ...
                            if (bar.valid()) {
                                ...
                                float baseWidth =
                                        Core.graphics.isPortrait()
                                                ? maxx - minx
                                                : (maxx - minx) - (maxy - y) - barpad * 2f - s * 4;
                                float cx = minx + barpad, cy = y, topY = cy + barheight, botY = cy;

                                Lines.square(
                                        cx + barheight / 2f, botY + barheight / 2f, barheight / 2f);
                                ...
                                font.draw(
                                        bar.text,
                                        cx + barheight * 1.5f,
                                        botY + barheight / 2f + layout.height / 2f);
                            }
                        }
                        ...
                        for (int i = 0; i < bars; i++) {
                            ...
                            float scale =
                                    Mathf.clamp(
                                            !renderTimes.hasEnoughData()
                                                    ? Mathf.randomSeed(i)
                                                    : (val / renderTimes.mean() - 0.5f));
                            ...
                            for (int j = 0; j < barmax; j++) {
                                ...
                                Fill.square(
                                        cx, miny + j * barspace + barw / 2f + barpad, barw / 2f);
                            }
                        }
                        Draw.color(color);
                        ...
                        int rx = (int) (vx + vw / 2f - vsize / 2f),
                                ry = (int) (vy + vh / 2f - vsize / 2f),
                                rw = (int) vsize,
                                rh = (int) vsize;
                        ...
                        if (!graphics.isPortrait()) {
                            ...
                            if (layout.width * 1.5f < vw) {
                                ...
                                for (int i = 0; i < points; i++) {
                                    ...
                                    Fill.poly(
                                            cx + Angles.trnsx(ang, vrad),
                                            cy + Angles.trnsy(ang, vrad),
                                            3,
                                            20 * s,
                                            ang);
                                }
                        ...
                        for (int tx = 0; tx < tris; tx++) {
                            for (int ty = 0; ty < tris; ty++) {
                                float trix = vx - trispace / 2f - trispace * tx - tpad,
                                        triy = vy + vh - trispace / 2f - trispace * ty - tpad;

                                Draw.color(
                                        Mathf.randomSeed(Pack.longInt(tx + 91, ty + 55))
                                                < 0.5 * (preview ? 1f : 1f - progress)
                                                ? colorRed
                                                : color);
        ...
        for (int i = 1; i < bars; i++) {
            ...
            float alpha =
                    progress >= fract ? 1f : Mathf.clamp((pscale - (fract - progress)) / pscale);
        ...
        // note for translators: this text is unreadable and for debugging/show anyway, so it's not
        // translated
        if (assets.isLoaded("tech")) {
            String name =
                    assets.getCurrentLoading() != null
                            ? assets.getCurrentLoading().fileName.toLowerCase()
                            : "system";
            ...
            String key =
                    name.contains("script")
                            ...
                            : name.contains("mod")
                            ? "mods"
                            : name.contains("msav") || name.contains("maps")
                            ? "map"
                            : name.contains("ogg") || name.contains("mp3")
                            ? "sound"
                            : name.contains("png")
                            ? "image"
                            : "system";
            ...
            font.draw(
                    red
                            ...
                            + Version.modifier
                            + "  "
                            + (Version.build == 0 ? "[init]" : Version.buildString())
                            + ">",
                    w / 2f,
                    h / 2f + 110 * s,
                    Align.center);
