...
public class EnvRenderers {
    ...
    public static void init() {
        ...
        Core.assets.load("sprites/rays.png", Texture.class).loaded =
                t -> {
        ...
        float windx = Mathf.cosDeg(windAngle) * windSpeed,
                windy = Mathf.sinDeg(windAngle) * windSpeed;

        renderer.addEnvRenderer(
                Env.underwater,
                () -> {
                    Draw.draw(
                            Layer.light + 1,
                    ...
                    for (int i = 0; i < rays; i++) {
                        ...
                        float x =
                                (rand.random(0f, world.unitWidth()) + (pos % 100) * 753)
                                        % world.unitWidth();
                        float y =
                                (rand.random(0f, world.unitHeight()) + (pos % 120) * 453)
                                        % world.unitHeight();
                        ...
                        if (opacity > 0.01) {
                            ...
                            Draw.rect(
                                    ...
                                    x,
                                    y + tex.height / 2f,
                                    tex.width * 2 * sizeScale,
                                    tex.height * 2 * sizeScale,
                                    rot);
                            Draw.color();
                        }
                    }
