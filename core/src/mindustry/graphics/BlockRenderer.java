...
public class BlockRenderer {
    ...
    public static final Color shadowColor = new Color(0, 0, 0, 0.71f),
            blendShadowColor = Color.white.cpy().lerp(Color.black, shadowColor.a);
                ...
                event -> {
                    ...
                    for (Tile tile : world.tiles) {
                        ...
                        if (tile.build != null
                                ...
                                || !state.rules.fog
                                || (tile.build.visibleFlags & (1L << player.team().id))
                                != 0)) {
                            tile.build.wasVisible = true;
                        }

                        if (tile.block().hasShadow
                                && (tile.build == null || tile.build.wasVisible)) {
                ...
                event -> {
                    boolean visible =
                            event.tile.build == null
                                    || !event.tile.build.inFogTo(Vars.player.team());
                    ...
                    if (Math.abs(avgx - event.tile.x) <= rangex
                            && Math.abs(avgy - event.tile.y) <= rangey) {
                        lastCamY =
                                lastCamX = -99; // invalidate camera position so blocks get updated
                    }
    ...
    public void updateDarkness() {
        ...
        for (Tile tile : world.tiles) {
            ...
            if (state.rules.limitMapArea
                    ...
                    state.rules.limitX,
                    state.rules.limitY,
                    state.rules.limitWidth - 1,
                    state.rules.limitHeight - 1,
                    tile.x,
                    tile.y)) {
                continue;
            }
    ...
    boolean indexFloor(Tile tile) {
        return tile.block() == Blocks.air
                ...
                && world.getDarkness(tile.x, tile.y) < 3;
    }
        ...
        if (!darkEvents.isEmpty()) {
            ...
            darkEvents.each(
                    pos -> {
                        ...
                        Draw.colorl(
                                darkness <= 0f ? 1f : 1f - Math.min((darkness + 0.5f) / 4f, 1f));
                        Fill.rect(tile.x + 0.5f, tile.y + 0.5f, 1, 1);
                    });
    ...
    public void drawDestroyed() {
        ...
        if (brokenFade > 0.001f) {
            for (BlockPlan block : player.team().data().plans) {
                ...
                if (!camera.bounds(Tmp.r1)
                        .grow(tilesize * 2f)
                        .overlaps(
                                Tmp.r2
                                        .setSize(b.size * tilesize)
                                        .setCenter(
                                                block.x * tilesize + b.offset,
                                                block.y * tilesize + b.offset))) continue;
                ...
                Draw.rect(
                        b.fullIcon,
                        block.x * tilesize + b.offset,
                        block.y * tilesize + b.offset,
                        b.rotate ? block.rotation * 90 : 0f);
            }
            Draw.reset();
        }
    }
    ...
    public void drawShadows() {
        if (!shadowEvents.isEmpty()) {
            ...
            for (Tile tile : shadowEvents) {
                ...
                Draw.color(
                        (!tile.block().hasShadow
                                || (state.rules.fog
                                && tile.build != null
                                && !tile.build.wasVisible))
                                ? Color.white
                                : blendShadowColor);
                Fill.rect(tile.x + 0.5f, tile.y + 0.5f, 1, 1);
            }
    ...
    public void processBlocks() {
        ...
        blockTree.intersect(
                ...
                tile -> {
                    ...
                    if (((tile.build != null && procLights.add(tile.build.pos()))
                            || tile.block().emitLight)) {
                        lightview.add(tile);
                    }

                    if (tile.build != null
                            && tile.build.power != null
                            && tile.build.power.links.size > 0) {
                        for (Building other : tile.build.getPowerConnections(outArray2)) {
                            if (other.block instanceof PowerNode
                                    && procLinks.add(
                                    other.id)) { // TODO need a generic way to render
                                // connections!
    ...
    public void drawBlocks() {
        ...
        for (int i = 0; i < tileview.size; i++) {
            ...
            if (block != Blocks.air && (visible || build.wasVisible)) {
                ...
                // TODO here is the question: should buildings you lost sight of remain rendered? if
                // so, how
            ...
            for (int i = 0; i < lightview.size; i++) {
                ...
                if (entity != null) {
                    ...
                    tile.block().drawEnvironmentLight(tile);
                        ...
                        && tile.block()
                        == Blocks.air) { // only draw floor light under non-solid blocks
