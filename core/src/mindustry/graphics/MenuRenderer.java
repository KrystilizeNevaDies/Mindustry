...
public class MenuRenderer implements Disposable {
    ...
    // no longer random or "dynamic", mod units in the menu look jarring, and it's not worth the
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                if (tech) {
                    ...
                    if (Simplex.noise2d(s1, 2, 1f / 10f, 0.5f, sclx, scly) > 0.4f
    ...
    public void render() {
        ...
        Draw.color();
        Draw.rect(
                Draw.wrap(shadows.getTexture()),
                width * tilesize / 2f - 4f,
                height * tilesize / 2f - 4f,
                width * tilesize,
                -height * tilesize);
        Draw.flush();
        batch.beginDraw();
    ...
    private void drawFlyers() {
        ...
        flyers(
                (x, y) -> {
                    float engineOffset = flyerType.engineOffset,
                            engineSize = flyerType.engineSize,
                            rotation = flyerRot;

                    Draw.color(Pal.engine);
                    Fill.circle(
                            x + Angles.trnsx(rotation + 180, engineOffset),
                            y + Angles.trnsy(rotation + 180, engineOffset),
                            engineSize + Mathf.absin(Time.time, 2f, engineSize / 4f));

                    Draw.color(Color.white);
                    Fill.circle(
                            x + Angles.trnsx(rotation + 180, engineOffset - 1f),
                            y + Angles.trnsy(rotation + 180, engineOffset - 1f),
    ...
    private void flyers(Floatc2 cons) {
        ...
        for (int i = 0; i < flyers; i++) {
            ...
            cons.get(
                    (Mathf.randomSeedRange(i, range)
                            + Tmp.v1.x
                            + Mathf.absin(
                            time + Mathf.randomSeedRange(i + 2, 500), 10f, 3.4f)
                            + offset)
                            % (tw + Mathf.randomSeed(i + 5, 0, 500)),
                    (Mathf.randomSeedRange(i + 1, range)
                            + Tmp.v1.y
                            + Mathf.absin(
                            time + Mathf.randomSeedRange(i + 3, 500), 10f, 3.4f)
                            + offset)
                            % th);
        }
    }
