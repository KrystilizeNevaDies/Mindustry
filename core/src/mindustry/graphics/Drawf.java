...
public class Drawf {
    ...
    public static void flame(
            float x, float y, int divisions, float rotation, float length, float width, float pan) {
        ...
        for (int i = 0; i < divisions; i++) {
            ...
            point(
                    (Tmp.v1.x + width)
                            / width
                            * len1, // convert to 0..1, then multiply by desired length
        ...
        for (int i = 0; i < divisions; i++) {
            ...
            point(
                    len1
                            + (Tmp.v1.x)
                            / width
                            * len2, // convert to 0..1, then multiply by desired length and
                    // offset relative to
                    // previous segment
                    Tmp.v1.y, // Y axis remains unchanged
                    x,
    ...
    public static void flameFront(
            float x, float y, int divisions, float rotation, float length, float width) {
        ...
        for (int i = 0; i <= divisions; i++) {
            ...
            point(
                    (Tmp.v1.x)
                            / width
                            * length, // convert to 0..1, then multiply by desired length and offset
                    // relative to
                    // previous segment
                    Tmp.v1.y, // Y axis remains unchanged
                    x,
    ...
    public static void buildBeam(float x, float y, float tx, float ty, float radius) {
        ...
        Arrays.sort(
                vecs,
                Structs.comparingFloat(
                        vec -> -Angles.angleDist(Angles.angle(x, y, vec.x, vec.y), ang)));
    ...
    public static void additive(
            TextureRegion region, Color color, float x, float y, float rotation) {
        additive(region, color, x, y, rotation, Layer.blockAdditive);
    }

    public static void additive(
            TextureRegion region, Color color, float x, float y, float rotation, float layer) {
    ...
    public static void dashLine(Color color, float x, float y, float x2, float y2) {
        dashLine(
                ...
                x2,
                y2,
                (int) (Math.max(Math.abs(x - x2), Math.abs(y - y2)) / tilesize * 2));
    }
    ...
    public static void dashLineBasic(float x, float y, float x2, float y2) {
        Lines.dashLine(
                x, y, x2, y2, (int) (Math.max(Math.abs(x - x2), Math.abs(y - y2)) / tilesize * 2));
    }
    ...
    public static void light(
            float x, float y, float x2, float y2, float stroke, Color tint, float alpha) {
        if (renderer == null) return;
    ...
    public static void selected(int x, int y, Block block, Color color) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            Draw.rect(
                    ...
                    y * tilesize + block.offset + offset * p.y,
                    i * 90);
        }
        Draw.reset();
    }
    ...
    public static void shadow(
            TextureRegion region, float x, float y, float width, float height, float rotation) {
        Draw.color(Pal.shadow);
    ...
    public static void liquid(
            TextureRegion region, float x, float y, float alpha, Color color, float rotation) {
        Draw.color(color, alpha * color.a);
        Draw.rect(region, x, y, rotation);
        Draw.color();
    }
    ...
    public static void arrow(
            float x, float y, float x2, float y2, float length, float radius, Color color) {
    ...
    public static void laser(
            TextureRegion line, TextureRegion edge, float x, float y, float x2, float y2) {
    ...
    public static void laser(
        ...
        Draw.rect(
                ...
                x,
                y,
                start.width * scale * Draw.scl,
                start.height * scale * Draw.scl,
                rot + 180);
    ...
    /**
     * Draws a sprite that should be light-wise correct, when rotated. Provided sprite must be
