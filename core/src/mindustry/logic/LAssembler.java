...
public class LAssembler {
    ...
    public static LAssembler assemble(String data, boolean privileged) {
        ...
        asm.instructions =
                st.map(l -> l.build(asm)).filter(l -> l != null).toArray(LInstruction.class);
        return asm;
    }
    ...
    /**
     * @return a variable ID by name. This may be a constant variable referring to a number or
     * object.
     */
    public int var(String symbol) {
        ...
        if (!symbol.isEmpty()
            ...
            return putConst(
                    "___" + symbol,
                    symbol.substring(1, symbol.length() - 1).replace("\\n", "\n"))
                    .id;
        }
