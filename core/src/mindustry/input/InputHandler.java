...
public abstract class InputHandler implements InputProcessor, GestureListener {
    ...
    public static void commandBuilding(Player player, Building build, Vec2 target) {
        if (player == null || build == null || build.team != player.team() || !build.block.commandable || target == null)
            return;
    ...
    public static void requestItem(Player player, Building build, Item item, int amount) {
        if (player == null || build == null || !build.interactable(player.team()) || !player.within(build, itemTransferRange) || player.dead())
            return;
