...
public class Administration {
    ...
    /**
     * All player info. Maps UUIDs to info. This persists throughout restarts. Do not access
     * directly.
     */
        ...
        addChatFilter(
                (player, message) -> {
                    ...
                    if (Config.antiSpam.bool() && !player.isLocal() && !player.admin) {
                        ...
                        if (resetTime > 0
                                && Time.timeSinceMillis(player.getInfo().lastMessageTime)
                                < resetTime) {
                            // supress message
                            player.sendMessage(
                                    "[scarlet]You may only send messages every "
                                            + Config.messageRateLimit.num()
                                            + " seconds.");
                            player.getInfo().messageInfractions++;
                            // kick player for spamming and prevent connection if they've done this
                            // several times
                            if (player.getInfo().messageInfractions >= Config.messageSpamKick.num()
                                    && Config.messageSpamKick.num() != 0) {
                                player.con.kick(
                                        "You have been kicked for spamming.", 1000 * 60 * 2);
                            }
                            return null;
                        } else {
                        ...
                        // prevent players from sending the same message twice in the span of 50
                        // seconds
        ...
        addActionFilter(
                action -> {
                    if (action.type != ActionType.breakBlock
                        ...
                        if (rate.allow(
                                Config.interactRateWindow.num() * 1000,
                                Config.interactRateLimit.num())) {
                            return true;
                        } else {
                            if (rate.occurences > Config.interactRateKick.num()) {
                                action.player.kick(
                                        "You are interacting with too many blocks.", 1000 * 30);
                            } else if (action.player.getInfo().messageTimer.get(60f * 2f)) {
                                action.player.sendMessage(
                                        "[scarlet]You are interacting with blocks too quickly.");
                            }
    ...
    /**
     * Adds a chat filter. This will transform the chat messages of every player. This functionality
     * can be used to implement things like swear filters and special commands. Note that commands
     * (starting with /) are not filtered.
     */
    ...
    public boolean allowAction(
            Player player, ActionType type, Tile tile, Cons<PlayerAction> setter) {
    ...
    /**
     * Bans a player by IP; returns whether this player was already banned. If there are players who
     * at any point had this IP, they will be UUID banned as well.
     */
    public boolean banPlayerIP(String ip) {
        if (bannedIPs.contains(ip, false)) return false;
    ...
    public boolean banPlayerID(String id) {
        if (playerInfo.containsKey(id) && playerInfo.get(id).banned) return false;
    ...
    /**
     * Unbans a player by IP; returns whether this player was banned in the first place. This method
     * also unbans any player that was banned and had this IP.
     */
    ...
    /**
     * Unbans a player by ID; returns whether this player was banned in the first place. This also
     * unbans all IPs the player used.
     */
    ...
    public ObjectSet<PlayerInfo> findByName(String name) {
        ...
        for (PlayerInfo info : playerInfo.values()) {
            if (info.lastName.equalsIgnoreCase(name)
                    || info.names.contains(name, false)
                    || Strings.stripColors(Strings.stripColors(info.lastName)).equals(name)
                    || info.ips.contains(name, false)
                    || info.id.equals(name)) {
                result.add(info);
            }
        }
    ...
    public ObjectSet<PlayerInfo> searchNames(String name) {
        ...
        for (PlayerInfo info : playerInfo.values()) {
            if (info.names.contains(
                    n ->
                            n.toLowerCase().contains(name.toLowerCase())
                                    || Strings.stripColors(n)
                                    .trim()
                                    .toLowerCase()
                                    .contains(name))) {
                result.add(info);
     ...
     * Creating a new Config instance implicitly adds it to the list of server configs. This can be
     * used for custom plugin configuration.
     */
    public static class Config {
        ...
        public static final Config
                ...
                strict =
                        new Config(
                                "strict",
                                "Whether strict mode is on - corrects positions and prevents duplicate UUIDs.",
                                true),
                antiSpam =
                        new Config(
                                "antiSpam",
                ...
                interactRateWindow =
                        new Config(
                                "interactRateWindow",
                                "Block interaction rate limit window, in seconds.",
                                6),
                interactRateLimit =
                        new Config("interactRateLimit", "Block interaction rate limit.", 25),
                interactRateKick =
                        new Config(
                                "interactRateKick",
                                "How many times a player must interact inside the window to get kicked.",
                                60),
                messageRateLimit =
                        new Config(
                                "messageRateLimit",
                                "Message rate limit in seconds. 0 to disable.",
                                0),
                messageSpamKick =
                        new Config(
                                "messageSpamKick",
                ...
                socketInput =
                        new Config(
                                "socketInput",
                                "Allows a local application to control this server through a local TCP socket.",
                                false,
                                "socket",
                ...
                socketInputPort =
                        new Config(
                                "socketInputPort",
                                "The port for socket input.",
                                6859,
                                () -> Events.fire(Trigger.socketConfigChanged)),
                socketInputAddress =
                        new Config(
                                "socketInputAddress",
                                "The bind address for socket input.",
                                "localhost",
                                () -> Events.fire(Trigger.socketConfigChanged)),
                allowCustomClients =
                        new Config(
                ...
                autosaveAmount =
                        new Config(
                                "autosaveAmount",
                                "The maximum amount of autosaves. Older ones get replaced.",
                                10),
                autosaveSpacing =
                        new Config(
    ...
    public interface ActionFilter {
        /**
         * @return whether this action should be permitted. if applicable, make sure to send this
         * player a message specify why the action was prohibited.
         */
    ...
    /**
     * Defines a (potentially dangerous) action that a player has done in the world. These objects
     * are pooled; do not cache them!
     */
