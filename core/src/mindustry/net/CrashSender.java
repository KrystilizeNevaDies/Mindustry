...
public class CrashSender {
    ...
    public static String createReport(String error) {
        ...
        return report
                ...
                + OS.osArch
                + ")\n"
                + ((OS.isAndroid || OS.isIos) && app != null
                ? "Android API level: " + Core.app.getVersion() + "\n"
                : "")
                + "Java Version: "
                + OS.javaVersion
                + "\n"
                ...
                ? "<no mod init>"
                : "Mods: "
                + (!mods.list().contains(LoadedMod::shouldBeEnabled)
                ? "none (vanilla)"
                : mods.list()
                .select(LoadedMod::shouldBeEnabled)
                .toString(
                        ", ",
                        mod -> mod.name + ":" + mod.meta.version)))
                + "\n\n"
                + error;
    }
    ...
    public static void log(Throwable exception) {
        try {
            Core.settings
                    .getDataDirectory()
                    .child("crashes")
                    .child("crash_" + System.currentTimeMillis() + ".txt")
    ...
    public static void send(Throwable exception, Cons<File> writeListener) {
        try {
                ...
                try {
                    ...
                    PropertiesUtils.load(
                            ...
                            new InputStreamReader(
                                    CrashSender.class.getResourceAsStream("/version.properties")));
                    ...
                    if (map.get("build").contains(".")) {
                        ...
                        Version.build =
                                Strings.canParseInt(map.get("build"))
                                        ? Integer.parseInt(map.get("build"))
                                        : -1;
                    }
                } catch (Throwable e) {
                    e.printStackTrace();
            ...
            try {
                File file =
                        new File(
                                OS.getAppDataDirectoryString(Vars.appName),
            ...
            try {
                ...
                // if there's no settings init we don't know what the user wants but chances are
                // it's an
                // important crash, so send it anyway
            }
