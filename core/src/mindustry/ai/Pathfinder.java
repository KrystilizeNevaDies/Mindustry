...
public class Pathfinder implements Runnable {
    ...
    public static final Seq<Prov<Flowfield>> fieldTypes = Seq.with(EnemyCoreField::new);

    public static final int costGround = 0, costLegs = 1, costNaval = 2;
            ...
            Seq.with(
                    ...
                    (team, tile) ->
                            (PathTile.allDeep(tile)
                                    || ((PathTile.team(tile) == team
                                    && !PathTile.teamPassable(tile))
                                    || PathTile.team(tile) == 0)
                                    && PathTile.solid(tile))
                                    ? impassable
                                    : 1
                    ...
                    // legs
                    (team, tile) ->
                            PathTile.legSolid(tile)
                                    ? impassable
                                    : 1
                                    + (PathTile.deep(tile) ? 6000 : 0)
                                    + // leg units can now drown
                                    (PathTile.solid(tile) ? 5 : 0),

                    // water
                    (team, tile) ->
                            (PathTile.solid(tile) || !PathTile.liquid(tile) ? 6000 : 1)
                                    + (PathTile.nearGround(tile) || PathTile.nearSolid(tile)
                                    ? 14
                                    : 0)
    ...
    Flowfield[][][] cache;
    /**
     * unordered array of path data for iteration only. DO NOT iterate or access this in the main
     * thread.
     */
                ...
                event -> {
                    ...
                    if (tile.solid()) {
                        for (int i = 0; i < 4; i++) {
                            ...
                            if (other != null) {
                                // other tile needs to update its nearSolid to be false if it's not
                                // solid and this
                                // tile just got un-solidified
                                if (!other.solid()) {
    ...
    public int packTile(Tile tile) {
        ...
        return PathTile.get(
                tile.build == null || !solid || tile.block() instanceof CoreBlock
                        ? 0
                        : Math.min((int) (tile.build.health / 40), 80),
                tid == 0 && tile.build != null && state.rules.coreCapture
                        ? 255
                        : tid, // use teamid = 255 when core capture is enabled to mark out derelict
                // structures
                solid,
                tile.floor().isLiquid,
    ...
    /**
     * Update a tile in the internal pathfinding grid. Causes a complete pathfinding recalculation.
     * Main thread only.
     */
    ...
    public @Nullable Tile getTargetTile(Tile tile, Flowfield path) {
        ...
        if (path.refreshRate > 0 && Time.timeSinceMillis(path.lastUpdateTime) > path.refreshRate) {
            ...
            synchronized (path.targets) {
                ...
                if (!(path.targets.size == 1
                        && tmpArray.size == 1
                        && path.targets.first() == tmpArray.first())) {
                    path.targets.clear();
                    path.getPositions(path.targets);
        ...
        for (Point2 point : Geometry.d8) {
            ...
            if (values[packed] < value
                    ...
                    && !(point.x != 0
                    && point.y != 0
                    && (!path.passable(world.packArray(tile.x + point.x, tile.y))
                    || !path.passable(
                    world.packArray(
                            tile.x,
                            tile.y + point.y))))) { // diagonal corner trap
                current = other;
                tl = values[packed];
            }
        }
        ...
        if (current == null
                || tl == impassable
                || (path.cost == costTypes.items[costGround]
                && current.dangerous()
                && !tile.dangerous())) return tile;

        return current;
    }

    /**
     * Clears the frontier, increments the search and sets up all flow sources. This only occurs for
     * active teams.
     */
    ...
    /**
     * TODO wrong docs Created a new flowfield that aims to get to a certain target for a certain
     * team. Pathfinding thread only.
     */
    ...
    private void updateFrontier(Flowfield path, long nsToRun) {
        ...
        while (path.frontier.size > 0) {
            ...
            // pathfinding overflowed for some reason, time to bail. the next block update will
            // handle
            // this, hopefully
            ...
            // every N iterations, check the time spent - this prevents extra calls to nano time,
            // which
            // itself is slow
    ...
    /**
     * Data for a flow field to some set of destinations. Concrete subclasses must specify a way to
     * fetch costs and destinations.
     */
    public abstract static class Flowfield {
        ...
        public int[] weights;
        /**
         * search IDs of each position - the highest, most recent search is prioritized and
         * overwritten
         */
        public int[] searches;
        /**
