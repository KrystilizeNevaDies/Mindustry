...
public class PathfindQueue {
     ...
     * queue[2*n+1] and queue[2*(n+1)]. The priority queue is ordered by the elements' natural
     * ordering: For each node n in the heap and each descendant d of n, n <= d. The element with
    ...
    /**
     * Inserts the specified element into this priority queue. If {@code uniqueness} is enabled and
     * this priority queue already contains the element, the call leaves the queue unchanged and
     * returns false.
     *
    ...
    /**
     * Retrieves, but does not remove, the head of this queue. If this queue is empty, {@code 0} is
     * returned.
     *
    ...
    private void growToSize(int minCapacity) {
        ...
        int newCapacity =
                (int)
                        ((oldCapacity < 64)
                                ? ((oldCapacity + 1) * CAPACITY_RATIO_HI)
