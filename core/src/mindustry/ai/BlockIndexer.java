...
public class BlockIndexer {
    ...
    public Building findTile(
            Team team, float x, float y, float range, Boolf<Building> pred, boolean usePriority) {
        ...
        for (int i = 0; i < breturnArray.size; i++) {
            ...
            if (bdst < range
                    ...
                    (bdst < dst
                            && (!usePriority
                            || closest.block.priority <= next.block.priority))
                    ||
                    // priority is used, and new block has higher priority regardless of
                    // range
    ...
    private void process(Tile tile) {
        ...
        if (tile.isCenter() && tile.build != null) {
            ...
            if (data.buildingTree == null) {
                data.buildingTree =
                        new QuadTree<>(new Rect(0, 0, world.unitWidth(), world.unitHeight()));
            }
