...
public class ControlPathfinder {
    ...
    costHover =
            (team, tile) ->
                    ...
                    PathTile.solid(tile)
                            ...
                            :
                            // impassable synthetic enemy block
                            ((PathTile.team(tile) != team && PathTile.team(tile) != 0)
                                    && PathTile.solid(tile)
                                    ? wallImpassableCap
                                    : 0)
                                    + 1
            ...
            costLegs =
                    (team, tile) ->
                            PathTile.legSolid(tile)
                                    ...
                                    : 1
                                    + (PathTile.deep(tile) ? 6000 : 0)
                                    + (PathTile.nearSolid(tile) || PathTile.solid(tile)
                                    ? 3
                                    : 0),
            costNaval =
                    (team, tile) ->
    ...
    public ControlPathfinder() {
        ...
        // only update the world when a solid block is removed or placed, everything else doesn't
        // matter
        Events.on(
    ...
    public boolean getPathPosition(Unit unit, int pathId, Vec2 destination, Vec2 out) {
        // uninitialized
        if (threads == null
                || !world.tiles.in(World.toTile(destination.x), World.toTile(destination.y)))
            return false;
        ...
        if ((!requests.containsKey(unit) || requests.get(unit).curId != pathId)
                && !raycast(
                team,
                costType,
                unit.tileX(),
                unit.tileY(),
                World.toTile(destination.x),
                World.toTile(destination.y))) {
            out.set(destination);
            return true;
        }
        ...
        if (solid(
                team,
                costType,
                world.packArray(World.toTile(destination.x), World.toTile(destination.y)))) {
            return false;
        }
        ...
        if (!requests.containsKey(unit)) {
            ...
            if (req.done) {
                ...
                float minDst =
                        req.pathIndex < len
                                ? unit.dst2(world.tiles.geti(items[req.pathIndex]))
                                : 0f;
                ...
                for (int i = len - 1; i >= idx; i--) {
                    ...
                    if (dst < minDst
                            && !permissiveRaycast(team, costType, tileX, tileY, tile.x, tile.y)) {
                ...
                if (req.rayPathIndex < len) {
                    ...
                    if (unit.within(tile, range)) {
                        req.pathIndex =
                                req.rayPathIndex = Math.max(req.pathIndex, req.rayPathIndex + 1);
                    }
                } else {
                    // implicit done
                    out.set(unit);
    ...
    private void start() {
        ...
        threads =
                new PathfindThread
                        [Mathf.clamp(Runtime.getRuntime().availableProcessors() - 2, 1, 6)];
    ...
    private static boolean permissiveRaycast(
            int team, PathCost type, int x1, int y1, int x2, int y2) {
        int ww = world.width(), wh = world.height();
        int x = x1, dx = Math.abs(x2 - x), sx = x < x2 ? 1 : -1;
        int y = y1, dy = Math.abs(y2 - y), sy = y < y2 ? 1 : -1;
        int err = dx - dy;
    ...
    private static int cost(int team, PathCost cost, int tilePos) {
        if (state.rules.limitMapArea && !Team.get(team).isAI()) {
            ...
            if (x < state.rules.limitX
                    || y < state.rules.limitY
                    || x > state.rules.limitX + state.rules.limitWidth
                    || y > state.rules.limitY + state.rules.limitHeight) {
                return impassable;
            }
        }
        return cost.getCost(team, pathfinder.tiles[tilePos]);
    ...
    static class PathRequest {
        ...
        void update(long maxUpdateNs) {
            ...
            if (Time.timeSinceMillis(lastTime) > 1000 * 3
                    && (worldUpdateId != lastWorldUpdate
                    || !destination.epsilonEquals(lastDestination, 2f))) {
            ...
            while (frontier.size > 0) {
                ...
                for (Point2 point : Geometry.d4) {
                    ...
                    // the cost can include an impassable enemy wall, so cap the cost if so and add
                    // the base
                    // cost instead
                    // essentially this means that any path with enemy walls will only count the
                    // walls once,
                    // preventing strange behavior like avoiding based on wall count
                    float newCost =
                            currentCost >= wallImpassableCap && add >= wallImpassableCap
                                    ? currentCost + add - wallImpassableCap
                                    : currentCost + add;
