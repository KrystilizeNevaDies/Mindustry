...
public class WaveSpawner {
    ...
    public boolean playerNear() {
        return state.hasSpawns()
                && !player.dead()
                && spawns.contains(
                g ->
                        Mathf.dst(g.x * tilesize, g.y * tilesize, player.x, player.y)
                ...
                eachFlyerSpawn(
                        ...
                        (spawnX, spawnY) -> {
                            for (int i = 0; i < spawned; i++) {
                                ...
                                unit.set(
                                        spawnX + Mathf.range(spread), spawnY + Mathf.range(spread));
                                spawnEffect(unit);
                            }
                        });
            } else {
    ...
    private void eachGroundSpawn(int filterPos, SpawnConsumer cons) {
        ...
        if (state.rules.attackMode
                && state.teams.isActive(state.rules.waveTeam)
            ...
            for (Building core : state.rules.waveTeam.cores()) {
                ...
                Tmp.v1
                        .set(firstCore)
                        .sub(core)
                        .limit(coreMargin + core.block.size * tilesize / 2f * Mathf.sqrt2);
    ...
    private void eachFlyerSpawn(int filterPos, Floatc2 cons) {
        for (Tile tile : spawns) {
            ...
            float spawnX =
                    Mathf.clamp(
                            world.width() * tilesize / 2f + Angles.trnsx(angle, trns),
                            -margin,
                            world.width() * tilesize + margin);
            float spawnY =
                    Mathf.clamp(
                            world.height() * tilesize / 2f + Angles.trnsy(angle, trns),
                            -margin,
                            world.height() * tilesize + margin);
    ...
    public void spawnEffect(Unit unit) {
        spawnEffect(
                unit, unit.angleTo(world.width() / 2f * tilesize, world.height() / 2f * tilesize));
    }
