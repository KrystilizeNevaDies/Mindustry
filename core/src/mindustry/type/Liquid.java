...
public class Liquid extends UnlockableContent implements Senseable {
    // must be static and global so conduits don't conflict - DO NOT INTERACT WITH THESE IN MODS OR
    // I
    // WILL PERSONALLY YELL AT YOU
    ...
    /**
     * 0-1, 0 is completely not flammable, anything above that may catch fire when exposed to heat,
     * 0.5+ is very flammable.
     */
    ...
    /**
     * how much heat this liquid can store. 0.4=water (decent), anything lower is probably less
     * dense and bad at cooling.
     */
    public float heatCapacity = 0.5f;
    /**
     * how thick this liquid is. 0.5=water (relatively viscous), 1 would be something like tar (very
     * slow).
     */
    ...
    public int getAnimationFrame() {
        return (int)
                (Time.time
                        / (gas ? animationScaleGas : animationScaleLiquid)
                        * animationFrames
                        + id * 5)
                % animationFrames;
    }
