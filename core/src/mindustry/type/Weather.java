...
public class Weather extends UnlockableContent {
    ...
    public void updateEffect(WeatherState state) {
        ...
        if (!headless && sound != Sounds.none) {
            float noise =
                    soundVolOscMag > 0
                            ? (float) Math.abs(Noise.rawNoise(Time.time / soundVolOscScl))
                            * soundVolOscMag
                            : 0;
    ...
    public static void drawSplashes(
            ...
            if (Tmp.r3.setCentered(x, y, life * 4f).overlaps(Tmp.r2)) {
                ...
                if (tile != null && tile.floor().liquidDrop == splasher) {
                    ...
                    for (int j : new int[]{-1, 1}) {
                        ...
                        Lines.lineAngle(
                                x + Tmp.v1.x, y + Tmp.v1.y, 90f + j * space, 3f * (1f - life));
                    }
                }
            }
        }
    ...
    public static void drawNoiseLayers(
            Texture noise,
            Color color,
            float noisescl,
            float opacity,
    ...
    public static void drawNoise(
        ...
        Tmp.tr1.set(
                Tmp.r1.x * scale,
                Tmp.r1.y * scale,
                (Tmp.r1.x + Tmp.r1.width) * scale,
                (Tmp.r1.y + Tmp.r1.height) * scale);
    ...
    public static void createWeather(
            Weather weather, float intensity, float duration, float windX, float windY) {
    ...
    public static class WeatherEntry {
        ...
        /**
         * Minimum and maximum spacing between weather events. Does not include the time of the
         * event itself.
         */
        public float minFrequency, maxFrequency, minDuration, maxDuration;
        /**
         * Cooldown time before the next weather event takes place This is *state*, not
         * configuration.
         */
    ...
    abstract static class WeatherStateComp implements Drawc, Syncc {
        ...
        public void draw() {
            if (renderer.weatherAlpha > 0.0001f
                    ...
                    && Core.settings.getBool("showweather")) {
                Draw.draw(
                        Layer.weather,
                        () -> {
