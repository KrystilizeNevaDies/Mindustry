...
public class Sector {
    ...
    public String displayThreat() {
        float step = 0.25f;
        String color =
                Tmp.c1.set(Color.white).lerp(Color.scarlet, Mathf.round(threat, step)).toString();
    ...
    /**
     * @return whether this sector can be landed on at all. Only sectors adjacent to non-wave
     * sectors can be landed on.
     */
    ...
    public void loadInfo() {
        info =
                Core.settings.getJson(
                        planet.name + "-s-" + id + "-info", SectorInfo.class, SectorInfo::new);

        // fix an old naming bug; this doesn't happen with new saves, but old saves need manual
        // fixes
    ...
    public boolean hasBase() {
        return save != null
                && info.hasCore
                && !(Vars.state.isGame() && Vars.state.rules.sector == this && state.gameOver);
    }
    ...
    public boolean hasEnemyBase() {
        return ((generateEnemyBase && preset == null)
                || (preset != null && preset.captureWave == 0))
    ...
    public TextureRegion icon() {
        return info.contentIcon != null
                ? info.contentIcon.uiIcon
    ...
    public void addItems(ItemSeq items) {
        ...
        if (isBeingPlayed()) {
            if (state.rules.defaultTeam.core() != null) {
                ...
                items.each(
                        (item, amount) ->
                                storage.add(item, Math.min(cap - storage.get(item), amount)));
            }
        } else if (hasBase()) {
            items.each(
                    (item, amount) ->
                            info.items.add(
                                    item,
                                    Math.min(info.storageCapacity - info.items.get(item), amount)));
    ...
    /**
     * Projects this sector onto a 4-corner square for use in map gen. Allocates a new object. Do
     * not call in the main loop.
     */
    private SectorRect makeRect() {
        ...
        Vec3 planeRight =
                plane.project(center.cpy().rotate(Vec3.Y, -4f)).sub(center).setLength(radius);
    ...
    public static class SectorRect {
        ...
        /**
         * Project a coordinate into 3D space. Both coordinates should be normalized to floats in
         * the range [0, 1]
         */
