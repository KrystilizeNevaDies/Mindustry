...
public class BuildWeapon extends Weapon {
    ...
    public void update(Unit unit, WeaponMount mount) {
        ...
        if (unit.activelyBuilding()) {
            ...
            mount.aimX =
                    unit.x
                            + Angles.trnsx(unit.rotation - 90, x, y)
                            + Angles.trnsx(weaponRotation, this.shootX, this.shootY);
            mount.aimY =
                    unit.y
                            + Angles.trnsy(unit.rotation - 90, x, y)
                            + Angles.trnsy(weaponRotation, this.shootX, this.shootY);
        }
    ...
    public void draw(Unit unit, WeaponMount mount) {
        ...
        if (unit.activelyBuilding()) {
            float z = Draw.z(),
                    ...
                    wx =
                            unit.x
                                    + Angles.trnsx(rotation, x, y)
                                    + Angles.trnsx(weaponRotation, 0, -mount.recoil),
                    wy =
                            unit.y
                                    + Angles.trnsy(rotation, x, y)
                                    + Angles.trnsy(weaponRotation, 0, -mount.recoil),
