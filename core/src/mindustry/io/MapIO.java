...
public class MapIO {
                                                ...
                                                for (int dx = 0; dx < size; dx++) {
                                                    for (int dy = 0; dy < size; dy++) {
                                                        int drawx = tile.x + dx + offsetx,
                                                                drawy = tile.y + dy + offsety;
                                                        walls.set(
                                                                drawx,
                                                                floors.height - 1 - drawy,
                                                                c);
                                                    }
                                                }
        ...
        for (int x = 0; x < pixmap.width; x++) {
            for (int y = 0; y < pixmap.height; y++) {
                ...
                pixmap.set(
                        x,
                        pixmap.height - 1 - y,
                        colorFor(tile.block(), tile.floor(), tile.overlay(), tile.team()));
            }
        }
        return pixmap;
    }
    ...
    public static int colorFor(Block wall, Block floor, Block overlay, Team team) {
        ...
        return (((Floor) overlay).wallOre
                ? overlay.mapColor
                : wall.solid
                ? wall.mapColor
                : !overlay.useColor ? floor.mapColor : overlay.mapColor)
                .rgba();
    }
    ...
    public static Pixmap writeImage(Tiles tiles) {
        ...
        for (Tile tile : tiles) {
            // while synthetic blocks are possible, most of their data is lost, so in order to avoid
            // questions like
            // "why is there air under my drill" and "why are all my conveyors facing right", they
            // are
            // disabled
            int color =
                    tile.block().hasColor && !tile.block().synthetic()
                            ? tile.block().mapColor.rgba()
                            : tile.floor().mapColor.rgba();
