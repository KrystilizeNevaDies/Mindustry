...
public abstract class SaveVersion extends SaveFileReader {
    ...
    /**
     * Registers a custom save chunk reader/writer by name. This is mostly used for mods that need
     * to save extra data.
     *
    ...
    public void writeCustomChunks(DataOutput stream, boolean net) throws IOException {
        var chunks =
                customChunks
                        ...
                        .select(
                                s ->
                                        customChunks.get(s).shouldWrite()
                                                && (!net || customChunks.get(s).writeNet()));
    ...
    public void writeMeta(DataOutput stream, StringMap tags) throws IOException {
        ...
        writeStringMap(
                ...
                StringMap.of(
                                ...
                                "height", world.height(),
                                "viewpos",
                                Tmp.v1.set(player == null ? Vec2.ZERO : player).toString(),
                                "controlledType",
                                headless || control.input.controlledType == null
                                        ? "null"
                                        : control.input.controlledType.name,
                                "nocores", state.rules.defaultTeam.cores().isEmpty(),
                                "playerteam",
                                player == null
                                        ? state.rules.defaultTeam.id
                                        : player.team().id)
                        .merge(tags));
    }
    ...
    public void readMeta(DataInput stream, WorldContext context) throws IOException {
        ...
        if (!headless) {
            ...
            control.input.controlledType =
                    content.getByName(ContentType.unit, map.get("controlledType", "<none>"));
    ...
    public void writeMap(DataOutput stream) throws IOException {
        ...
        for (int i = 0; i < world.width() * world.height(); i++) {
            ...
            for (int j = i + 1; j < world.width() * world.height() && consecutives < 255; j++) {
                ...
                if (nextTile.floorID() != tile.floorID()
                        || nextTile.overlayID() != tile.overlayID()) {
                    break;
                }
    ...
    public void readContentHeader(DataInput stream) throws IOException {
        ...
        for (int i = 0; i < mapped; i++) {
            ...
            for (int j = 0; j < total; j++) {
                ...
                map[type.ordinal()][j] =
                        content.getByName(
                                type, type == ContentType.block ? fallback.get(name, name) : name);
            }
        }
