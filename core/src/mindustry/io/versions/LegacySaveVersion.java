        ...
        try {
            ...
            for (int i = 0; i < width * height; i++) {
                ...
                boolean occupied =
                        tile.build != null
                                && !tile.isCenter()
                                && (tile.build.block == block || block == Blocks.air);
                    ...
                    try {
                        readChunk(
                                ...
                                in -> {
                                    ...
                                    byte team =
                                            Pack.leftByte(packedrot) == 8
                                                    ? stream.readByte()
                                                    : Pack.leftByte(packedrot);
                                    ...
                                    if (tile.build.items != null)
                                        tile.build.items.read(Reads.get(stream), true);
                                    if (tile.build.power != null)
                                        tile.build.power.read(Reads.get(stream), true);
                                    if (tile.build.liquids != null)
                                        tile.build.liquids.read(Reads.get(stream), true);
