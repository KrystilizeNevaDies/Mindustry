...
public abstract class SaveFileReader {
    public static final ObjectMap<String, String> fallback =
            ObjectMap.of(
    ...
    public static final ObjectMap<String, String> modContentNameMap =
            ObjectMap.of(
                    "craters", "crater-stone",
                    "deepwater", "deep-water",
                    "water", "shallow-water",
    ...
    public void region(
            String name, DataInput stream, CounterInputStream counter, IORunner<DataInput> cons)
            throws IOException {
        counter.resetCount();
    ...
    public void region(String name, DataOutput stream, IORunner<DataOutput> cons)
            throws IOException {
        try {
    ...
    /**
     * Write a chunk of input to the stream. An integer of some length is written first, followed by
     * the data.
     */
    public void writeChunk(DataOutput output, boolean isShort, IORunner<DataOutput> runner)
            throws IOException {
        ...
        ReusableByteOutStream dout =
                isShort ? byteOutputSmall : wasNested ? byteOutput2 : byteOutput;
        try {
            // reset output position
            dout.reset();
            // write the needed info
            runner.accept(isShort ? dataBytesSmall : wasNested ? dataBytes2 : dataBytes);
     ...
     */
    public int readChunk(DataInput input, boolean isShort, IORunner<DataInput> runner)
            throws IOException {
    ...
    public void skipChunk(DataInput input, boolean isShort) throws IOException {
        ...
        if (length != skipped) {
            throw new IOException(
                    "Could not skip bytes. Expected length: "
                            + length
                            + "; Actual length: "
                            + skipped);
        }
    ...
    public void writeStringMap(DataOutput stream, ObjectMap<String, String> map)
            throws IOException {
    ...
    public abstract void read(
            DataInputStream stream, CounterInputStream counter, WorldContext context)
            throws IOException;
