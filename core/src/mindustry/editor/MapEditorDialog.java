...
public class MapEditorDialog extends Dialog implements Disposable {
            ...
            menu.cont
                    .button(
                            ...
                            () -> {
                                ...
                                if (editor.tags.containsKey("steamid")
                                        && builtin != null
                                        && !builtin.custom) {
                    ...
                    .update(
                            b ->
                                    b.setText(
                                            editor.tags.containsKey("steamid")
                                                    ? editor.tags
                                                    .get("author", "")
                                                    .equals(steamPlayerName)
                                                    ? "@workshop.listing"
                                                    : "@view.workshop"
                                                    : "@editor.publish.workshop"));
    ...
    private void editInGame() {
        ...
        ui.loadAnd(
                () -> {
                    ...
                    hide();
                    // only reset the player; logic.reset() will clear entities, which we do not
                    // want
                    ...
                    if (player.team().core() == null) {
                        ...
                        var unit =
                                (state.rules.hasEnv(Env.scorching)
                                        ? UnitTypes.evoke
                                        : UnitTypes.alpha)
                                        .spawn(player.team(), player.x, player.y);
        ...
        if (map != null) {
            ...
            if (Core.input.shift()) {
                ...
                control.playMap(
                        map,
                        map.applyRules(
                                Gamemode.survival.valid(map)
                                        ? Gamemode.survival
                                        : Gamemode.attack.valid(map)
                                        ? Gamemode.attack
                                        : Gamemode.sandbox),
                        true);
            } else {
    ...
    /**
     * Argument format: 0) button name 1) description 2) icon name 3) listener
     */
                ...
                cont -> {
                    ...
                    cont.table(
                                    mid -> {
                                        ...
                                        Cons<EditorTool> addTool =
                                                tool -> {
                                                    ImageButton button =
                                                            new ImageButton(
                                                                    ui.getIcon(tool.name()),
                                        ...
                                        ButtonGroup<ImageButton> teamgroup =
                                                new ButtonGroup<>();
    ...
    private void addBlockSelection(Table cont) {
        ...
        cont.table(
                        Tex.underline,
                        extra ->
                                extra.labelWrap(() -> editor.drawBlock.localizedName)
                                        .width(200f)
                                        .center())
                .growX();
        cont.row();
    ...
    private void rebuildBlockSelection(String searchText) {
        ...
        blocksOut.sort(
                (b1, b2) -> {
                    ...
                    int ore =
                            Boolean.compare(b1 instanceof OverlayFloor, b2 instanceof OverlayFloor);
                    if (ore != 0) return ore;
