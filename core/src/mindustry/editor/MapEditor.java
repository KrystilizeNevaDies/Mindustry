...
public class MapEditor {
        ...
        for (int i = 0; i < tiles.width * tiles.height; i++) {
            ...
            tiles.seti(
                    ...
                    new EditorTile(
                            ...
                            tile.floorID(),
                            tile.overlayID(),
                            build == null ? tile.blockID() : 0));
        }

        for (var build : builds) {
            tiles.get(build.tileX(), build.tileY())
                    .setBlock(build.block, build.team, build.rotation, () -> build);
        }
    ...
    boolean hasOverlap(int x, int y) {
        ...
        if (tile != null
                && tile.isCenter()
                && tile.block() != drawBlock
                && tile.block().size == drawBlock.size
                && tile.x == x
                && tile.y == y) {
            return false;
        }
            ...
            for (int y = 0; y < height; y++) {
                ...
                if (previous.in(px, py)) {
                    ...
                    if (tile.build != null && tile.isCenter()) {
                        ...
                        if (config != null) {
                            Object out =
                                    BuildPlan.pointConfig(
                                            tile.block(), config, p -> p.sub(offsetX, offsetY));
                            if (out != config) {
