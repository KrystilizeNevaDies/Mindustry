    ...
    CachedTile ctile =
            new CachedTile() {
                ...
                public void setBlock(
                        Block type, Team team, int rotation, Prov<Building> entityprov) {
                    this.block = type;
                }
            };
        ...
        for (GenerateFilter filter : filters) {
            ...
            for (int x = 0; x < editor.width(); x++) {
                for (int y = 0; y < editor.height(); y++) {
                    ...
                    filter.apply(input);
                    writeTiles[x + y * world.width()] =
                            PackTile.get(input.block.id, input.floor.id, input.overlay.id);
                }
            }
            ...
            editor.load(
                    () -> {
                        ...
                        for (int i = 0; i < editor.width() * editor.height(); i++) {
                            ...
                            Block block = content.block(PackTile.block(write)),
                                    floor = content.block(PackTile.floor(write)),
                                    overlay = content.block(PackTile.overlay(write));
        ...
        cont.table(
                        t -> {
                            ...
                            t.stack(
                                    ...
                                    .grow()
                                    .padRight(10);
                            t.pane(p -> filterTable = p.marginRight(6))
                                    .update(
                                            pane -> {
                                                if (Core.scene.getKeyboardFocus() instanceof Dialog
                                                        && Core.scene.getKeyboardFocus() != this) {
                                                    return;
                                                }

                                                Vec2 v =
                                                        pane.stageToLocalCoordinates(
                                                                Core.input.mouse());
                ...
                .pane(
                        p -> {
                            ...
                            p.button(
                                            Iconc.refresh
                                                    + " "
                                                    + Core.bundle.get("filter.defaultores"),
                                            Styles.flatt,
                                            () -> {
                        ...
                        () -> {
                            try {
                                ...
                                if (!filters.isEmpty()) {
                                    ...
                                    for (int px = 0; px < pixmap.width; px++) {
                                        for (int py = 0; py < pixmap.height; py++) {
                                            buffer1[px + py * w] =
                                                    pack(editor.tile(px * scaling, py * scaling));
                                        }
                                    }
                                }
                                ...
                                for (var filter : copy) {
                                    ...
                                    pixmap.each(
                                            (px, py) -> {
                                                ...
                                                input.set(
                                                        x,
                                                        y,
                                                        content.block(PackTile.block(tile)),
                                                        content.block(PackTile.floor(tile)),
                                                        content.block(PackTile.overlay(tile)));
                                                filter.apply(input);
                                                buffer2[px + py * w] =
                                                        PackTile.get(
                                                                input.block.id,
                                    ...
                                    pixmap.each(
                                            (px, py) ->
                                                    buffer1[px + py * w] = buffer2[px + py * w]);
                                }

                                for (int px = 0; px < pixmap.width; px++) {
                                    for (int py = 0; py < pixmap.height; py++) {
                                        int color;
                                        // get result from buffer1 if there's filters left,
