...
public class MapRenderer implements Disposable {
    ...
    public void resize(int width, int height) {
        ...
        chunks =
                new IndexedRenderer[(int) Math.ceil((float) width / chunkSize)]
                        [(int) Math.ceil((float) height / chunkSize)];
    ...
    public void draw(float tx, float ty, float tw, float th) {
        ...
        for (int x = 0; x < chunks.length; x++) {
            for (int y = 0; y < chunks[0].length; y++) {
                ...
                mesh.getTransformMatrix()
                        .setToTranslation(tx, ty)
                        .scale(tw / (width * tilesize), th / (height * tilesize));
    ...
    private TextureRegion getIcon(Block wall, int index) {
        return !wall.editorIcon().found()
                ? clearEditor
                : wall.variants > 0
                ? wall.editorVariantRegions()[
                Mathf.randomSeed(index, 0, wall.editorVariantRegions().length - 1)]
                : wall.editorIcon();
    }
    ...
    private void render(int wx, int wy) {
        ...
        if (wall != Blocks.air && useSyntheticWall) {
            ...
            float width = region.width * Draw.scl,
                    height = region.height * Draw.scl,
                    ox = wall.offset + (tilesize - width) / 2f,
                    oy = wall.offset + (tilesize - height) / 2f;
            ...
            region =
                    floor.editorVariantRegions()[
                            Mathf.randomSeed(idxWall, 0, floor.editorVariantRegions().length - 1)];
        ...
        if ((wall.update || wall.destructible) && center) {
            ...
            region =
                    overlay.editorVariantRegions()[
                            Mathf.randomSeed(
                                    idxWall, 0, tile.overlay().editorVariantRegions().length - 1)];
        } else {
            region = clearEditor;
        }
        ...
        mesh.draw(
                idxDecal, region, wx * tilesize + offsetX, wy * tilesize + offsetY, width, height);
