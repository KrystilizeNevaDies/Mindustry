...
public class SoundControl {
    public float finTime = 120f,
            foutTime = 120f,
            musicInterval = 3f * Time.toMinutes,
            musicChance = 0.6f,
            musicWaveChance = 0.46f;
    ...
    public SoundControl() {
        ...
        Events.on(
                ...
                e ->
                        Time.run(
                                Mathf.random(8f, 15f) * 60f,
                                () -> {
                                    boolean boss =
                                            state.rules.spawns.contains(
                                                    group ->
                                                            group.getSpawned(state.wave - 2) > 0
                                                                    && group.effect
                                                                    == StatusEffects.boss);
    ...
    protected void updateLoops() {
        ...
        sounds.each(
                (sound, data) -> {
                    ...
                    float pan =
                            Mathf.zero(data.total, 0.0001f)
                                    ? 0f
                                    : sound.calcPan(
                                    data.sum.x / data.total, data.sum.y / data.total);
    ...
    /**
     * Plays and fades in a music track. This must be called every frame. If something is already
     * playing, fades out that track and fades in this new music.
     */
