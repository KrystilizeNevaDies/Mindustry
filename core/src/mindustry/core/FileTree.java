...
public class FileTree implements FileHandleResolver {
    ...
    /**
     * Loads a sound by name from the sounds/ folder. OGG and MP3 are supported; the extension is
     * automatically added to the end of the file name. Results are cached; consecutive calls to
     * this method with the same name will return the same sound instance.
     */
    public Sound loadSound(String soundName) {
        if (Vars.headless) return Sounds.none;

        return loadedSounds.get(
                soundName,
                () -> {
                    String name = "sounds/" + soundName;
                    String path =
                            Vars.tree.get(name + ".ogg").exists() ? name + ".ogg" : name + ".mp3";
    ...
    /**
     * Loads a music file by name from the music/ folder. OGG and MP3 are supported; the extension
     * is automatically added to the end of the file name. Results are cached; consecutive calls to
     * this method with the same name will return the same music instance.
     */
