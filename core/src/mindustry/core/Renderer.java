...
public class Renderer implements ApplicationListener {
    ...
    private static final float cloudScaling = 1700f,
            cfinScl = -2f,
            cfinOffset = 0.3f,
            calphaFinOffset = 0.25f;
    private static final float[] cloudAlphas = {0, 0.5f, 1f, 0.1f, 0, 0f};
    private static final float cloudAlpha = 0.81f;
    private static final float[] thrusterSizes = {
            0f, 0f, 0f, 0f, 0.3f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 0f
    };
    ...
    public void init() {
        ...
        assets.load("sprites/clouds.png", Texture.class).loaded =
                t -> {
        ...
        Events.on(
                ...
                e -> {
                    // reset background buffer on every world load, so it can be re-cached first
                    // render
    ...
    public void draw() {
        ...
        Draw.drawRange(
                Layer.blockBuilding, () -> Draw.shader(Shaders.blockbuild, true), Draw::shader);
        ...
        if (animateShields && Shaders.shield != null) {
            // TODO would be nice if there were a way to detect if any shields or build beams
            // actually
            // *exist* before beginning/ending buffers, otherwise you're just blitting and swapping
            // shaders for nothing
            Draw.drawRange(
                    Layer.shields,
                    1f,
    ...
    protected void drawBackground() {
        ...
        if (state.rules.backgroundTexture != null
                ...
                || !state.rules.planetBackground.drawSkybox)) {
        ...
        if (state.rules.planetBackground != null) {
            ...
            Draw.rect(
                    Draw.wrap(backgroundBuffer.getTexture()),
                    camera.position.x,
                    camera.position.y,
                    drawSize,
                    -drawSize);
    ...
    void updateLandParticles() {
        ...
        if (landCore != null && landPTimer >= 1f) {
            landCore.tile.getLinkedTiles(
                    t -> {
                        if (Mathf.chance(0.4f)) {
                            Fx.coreLandDust.at(
                                    t.worldx(),
                                    t.worldy(),
                                    landCore.angleTo(t) + Mathf.range(30f),
    ...
    void drawLanding() {
        ...
        if (landTime > 0 && build != null) {
            ...
            // draw core
            var block =
                    launching && launchCoreType != null ? launchCoreType : (CoreBlock) build.block;
            ...
            float rotation = Interp.pow2In.apply(fout) * 135f,
                    x = build.x + Mathf.range(shake),
                    y = build.y + Mathf.range(shake);
            ...
            // draw thruster flame
            float strength =
                    (1f + (block.size - 3) / 2.5f)
                            * scl
                            * thrusterSize
                            * (0.95f + Mathf.absin(2f, 0.1f));
            ...
            for (int i = 0; i < 4; i++) {
                ...
                Tmp.v1.setLength(
                        (block.size * tilesize / 2f + 1f) * scl + strength * 0.5f + offset);
            ...
            if (state.rules.cloudColor.a > 0.0001f) {
                ...
                float sscl =
                        Math.max(1f + Mathf.clamp(fin + cfinOffset) * cfinScl, 0f) * camerascale;
                ...
                Draw.rect(
                        Tmp.tr1, camera.position.x, camera.position.y, camera.width, camera.height);
                Draw.reset();
            }
        }
    }
        ...
        Threads.thread(
                () -> {
                    ...
                    app.post(
                            () ->
                                    ui.showInfoFade(
                                            Core.bundle.format("screenshot", file.toString())));
                });
    }
