...
public final class FogControl implements CustomChunk {
    ...
    public FogControl() {
        ...
        Events.on(
                ...
                e -> {
                    if (state.rules.fog
                            && e.tile.build != null
                            && !e.tile.build.team.isOnlyAI()
                            && e.tile.block().flags.contains(BlockFlag.hasFogRadius)) {
            ...
            if (!team.team.isOnlyAI()) {
                ...
                synchronized (staticEvents) {
                    ...
                    for (var unit : team.units) {
                        ...
                        // always update the dynamic events, but only *flush* the results when
                        // necessary?
                ...
                if (data.dynamicUpdated
                        && Time.timeSinceMillis(data.lastDynamicMs) > dynamicUpdateInterval) {
    ...
    void updateStatic() {
        ...
        // I really don't like synchronizing here, but there should be *some* performance benefit at
        // least
        synchronized (staticEvents) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                int x = FogEvent.x(event),
                        y = FogEvent.y(event),
                        rad = FogEvent.radius(event),
                        team = FogEvent.team(event);
                var data = fog[team];
                if (data != null) {
