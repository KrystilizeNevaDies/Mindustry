...
public class SectorInfo {
    ...
    public Sector getRealDestination() {
        // on multiplayer the destination is, by default, the first captured sector (basically
        // random)
        return !net.client() || destination != null
                ? destination
                : state.rules.sector.planet.sectors.find(Sector::hasBase);
    }
    ...
    public void write() {
        ...
        if (state.rules.sector != null
                && state.rules.sector.preset != null
                && state.rules.sector.preset.captureWave > 0
                && !state.rules.sector.planet.allowWaves) {
        ...
        if (entity != null) {
            ...
            // ensure capacity.
            entity.items.each(
                    (i, a) -> entity.items.set(i, Mathf.clamp(a, 0, entity.storageCapacity)));
        }
    }
    ...
    public void prepare() {
        ...
        bestCoreType =
                !hasCore
                        ? Blocks.air
                        : state.rules.defaultTeam.cores().max(e -> e.block.size).block;
        ...
        if (time.get(refreshPeriod)) {
            ...
            export.each(
                    (item, stat) -> {
                        ...
                        // add counter, subtract how many items were taken from the core during this
                        // time
            ...
            for (Item item : content.items()) {
                ...
                production.get(item).mean =
                        Math.min(production.get(item).mean, rawProduction.get(item).mean);

                if (export.containsKey(item)) {
                    // export can, at most, be the raw items being produced from factories + the
                    // items being
                    // taken from the core
                    export.get(item).mean =
                            Math.min(
                                    export.get(item).mean,
                                    rawProduction.get(item).mean
                                            + Math.max(-production.get(item).mean, 0));
                }
            }
