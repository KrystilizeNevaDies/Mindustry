...
public class KeybindDialog extends Dialog {
    ...
    private void setup() {
        ...
        for (Section section : sections) {
                ...
                TextButton button =
                        new TextButton(
                                bundle.get(
                                        ...
                                        Strings.capitalize(section.name)));
                if (section.equals(this.section)) button.toggle();
            ...
            stable.button(
                    ...
                    .disabled(sectionControls.get(section, 0) - 1 < 0)
                    .size(40);
            ...
            stable.button(
                    ...
                    .disabled(sectionControls.get(section, 0) + 1 >= devices.size)
                    .size(40);
            ...
            if (section.device.type() == DeviceType.controller) {
                table.table(
                        info ->
                                info.add(
                                                "Controller Type: [lightGray]"
                                                        + Strings.capitalize(section.device.name()))
                                        .left());
            }
            table.row();
            ...
            for (KeyBind keybind : keybinds.getKeybinds()) {
                if (lastCategory != keybind.category() && keybind.category() != null) {
                    table.add(
                                    bundle.get(
                                            "category." + keybind.category() + ".name",
                                            Strings.capitalize(keybind.category())))
                            .color(Color.gray)
                            .colspan(4)
                            .pad(10)
                            .padBottom(4)
                ...
                if (keybind.defaultValue(section.device.type()) instanceof Axis) {
                    table.add(
                                    bundle.get(
                                            "keybind." + keybind.name() + ".name",
                                            Strings.capitalize(keybind.name())),
                                    Color.white)
                            .left()
                            .padRight(40)
                            .padLeft(8);
                    ...
                    table.labelWrap(
                                    () -> {
                                        ...
                                        return axis.key != null
                                                ? axis.key.toString()
                                                : axis.min + " [red]/[] " + axis.max;
                                    })
                            .color(Pal.accent)
                            .left()
                            .minWidth(90)
            ...
            table.button("@settings.reset", () -> keybinds.resetToDefaults())
                    .colspan(4)
                    .padTop(4)
                    .fill();
    ...
    void rebind(Section section, KeyBind bind, KeyCode newKey) {
        ...
        if (isAxis) {
            if (newKey.axis || !rebindMin) {
                section.binds
                        .get(section.device.type(), OrderedMap::new)
                        .put(rebindKey, newKey.axis ? new Axis(newKey) : new Axis(minKey, newKey));
            }
        } else {
            section.binds
                    .get(section.device.type(), OrderedMap::new)
                    .put(rebindKey, new Axis(newKey));
        }
    ...
    private void openDialog(Section section, KeyBind name) {
        rebindDialog =
                new Dialog(
                        rebindAxis
                                ? bundle.get("keybind.press.axis")
                                : bundle.get("keybind.press"));
