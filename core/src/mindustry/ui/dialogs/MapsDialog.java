                                                ...
                                                () -> {
                                                    maps.tryCatchMapError(
                                                            () -> {
                                                                ...
                                                                Map map =
                                                                        MapIO.createMap(file, true);

                                                                // when you attempt to import a
                                                                // save, it will have no name, so
                                                                // generate one
                                                                String name =
                                                                        map.tags.get(
                                                                                "name",
                                                                ...
                                                                // this will never actually get
                                                                // called, but it remains just in
                                                                // case
                                                                if (name == null) {
                                                                    ui.showErrorMessage(
                                                                            "@editor.errorname");
        ...
        if (mapList != null) {
            for (Map map : mapList) {
                ...
                if (invalid
                        ...
                        && (!searchAuthor
                        || !Strings.stripColors(map.author())
                        .toLowerCase()
                        .contains(searchString))
                        && (!searchDescription
                        || !Strings.stripColors(map.description())
                        .toLowerCase()
                        .contains(searchString)))) {
                    continue;
                }
                ...
                TextButton button =
                        mapTable.button("", Styles.grayt, () -> showMapInfo(map))
                                .width(mapsize)
                                .pad(8)
                                .get();
                ...
                button.row();
                button.stack(
                                new Image(map.safeTexture()).setScaling(Scaling.fit),
                                new BorderImage(map.safeTexture()).setScaling(Scaling.fit))
                        .size(mapsize - 20f);
                button.row();
                button.add(
                                map.custom
                                        ...
                                        ? "@workshop"
                                        : map.mod != null
                                        ? "[lightgray]" + map.mod.meta.displayName()
                                        : "@builtin")
                        .color(Color.gray)
                        .padTop(3);
                            ...
                            t -> {
                                t.button(
                                                ...
                                                () -> {
                                                    ...
                                                    Core.settings.put(
                                                            "editorsearchauthor", searchAuthor);
                                                    Core.settings.forceSave();
                                                    rebuildMaps();
                                                })
                                        .size(150f, 60f)
    ...
    void showMapInfo(Map map) {
        ...
        table.table(
                        ...
                        desc -> {
                            ...
                            t.row();
                            t.add(
                                            !map.custom && map.tags.get("author", "").isEmpty()
                                                    ? "Anuke"
                                                    : map.author())
                                    .growX()
                                    .wrap()
        ...
        table.button(
                        map.workshop && steam ? "@view.workshop" : "@delete",
                        map.workshop && steam ? Icon.link : Icon.trash,
                        () -> {
                            if (map.workshop && steam) {
                                ...
                                ui.showConfirm(
                                        "@confirm",
                                        Core.bundle.format("map.delete", map.name()),
                                        () -> {
                                            maps.removeMap(map);
                                            dialog.hide();
                                            setup();
