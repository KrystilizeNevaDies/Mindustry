...
public class ResearchDialog extends BaseDialog {
                ...
                () -> {
                    ...
                    items =
                            new ItemSeq() {
                                ...
                                // this is the only method that actually modifies the sequence
                                // itself.
                                @Override
                                public void add(Item item, int amount) {
                                    // only have custom removal logic for when the sequence gets
                                    // items taken out of it
                                    // (e.g. research)
                                    if (amount < 0) {
                                        // remove items from each sector's storage, one by one

                                        // negate amount since it's being *removed* - this makes it
                                        // positive
                                        amount = -amount;
        ...
        addListener(
                new InputListener() {
                    ...
                    public boolean scrolled(
                        ...
                        view.setScale(
                                Mathf.clamp(view.scaleX - amountY / 10f * view.scaleX, 0.25f, 1f));
        ...
        addCaptureListener(
                new ElementGestureListener() {
                    ...
                    public void zoom(InputEvent event, float initialDistance, float distance) {
                        ...
                        view.setScale(
                                Mathf.clamp(distance / initialDistance * view.lastZoom, 0.25f, 1f));
                        view.setOrigin(Align.center);
                        view.setTransform(true);
                    }
                    ...
                    @Override
                    public void pan(
                            InputEvent event, float x, float y, float deltaX, float deltaY) {
    ...
    void treeLayout() {
        ...
        LayoutNode[] leftHalf =
                Arrays.copyOfRange(node.children, 0, Mathf.ceil(node.children.length / 2f));
        LayoutNode[] rightHalf =
                Arrays.copyOfRange(
                        node.children, Mathf.ceil(node.children.length / 2f), node.children.length);
    ...
    class LayoutNode extends TreeNode<LayoutNode> {
        ...
        LayoutNode(TechTreeNode node, LayoutNode parent) {
            ...
            if (node.children != null) {
                children =
                        Seq.with(node.children)
                                .map(t -> new LayoutNode(t, this))
    ...
    public class View extends Group {
        ...
        public void rebuildAll() {
            ...
            for (TechTreeNode node : nodes) {
                ...
                button.update(
                        () -> {
                            ...
                            button.setPosition(
                                    node.x + panX + width / 2f,
                                    node.y + panY + height / 2f + offset,
                                    Align.center);
                            button.getStyle().up =
                                    !locked(node.node)
                                            ? Tex.buttonOver
                                            : !selectable(node.node) || !canSpend(node.node)
                                            ? Tex.buttonRed
                                            : Tex.button;
                            ...
                            ((TextureRegionDrawable) button.getStyle().imageUp)
                                    .setRegion(
                                            node.selectable
                                                    ? node.node.content.uiIcon
                                                    : Icon.lock.getRegion());
                            button.getImage()
                                    .setColor(
                                            !locked(node.node)
                                                    ? Color.white
                                                    : node.selectable ? Color.gray : Pal.gray);
            ...
            if (mobile) {
                tapped(
                        () -> {
                            Element e =
                                    Core.scene.hit(Core.input.mouseX(), Core.input.mouseY(), true);
                            if (e == this) {
                                hoverNode = null;
                                rebuild();
                            }
        ...
        void spend(TechNode node) {
            ...
            for (int i = 0; i < node.requirements.length; i++) {
                ...
                int used =
                        Math.max(Math.min(req.amount - completed.amount, items.get(req.item)), 0);
            ...
            infoTable.table(
                    b -> {
                        ...
                        if (selectable
                                ...
                                || node.content.stats.toMap().size > 0)) {
                            b.button(Icon.info, Styles.flati, () -> ui.content.show(node.content))
                                    .growY()
                                    .width(50f);
                        }
                        b.add().grow();
                                                ...
                                                desc.table(
                                                        t -> {
                                                            ...
                                                            if (selectable) {
                                                                ...
                                                                // add research progress text
                                                                if (Structs.contains(
                                                                        node.finishedRequirements,
                                                                        s -> s.amount > 0)) {
                ...
                infoTable
                        .table(
                                t ->
                                        t.margin(3f)
                                                .left()
                                                .labelWrap(node.content.displayDescription())
