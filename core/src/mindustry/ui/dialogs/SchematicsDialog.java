...
public class SchematicsDialog extends BaseDialog {
    ...
    public SchematicsDialog() {
        super("@schematics");
        Core.assets.load("sprites/schematic-background.png", Texture.class).loaded =
                t -> t.setWrap(TextureWrap.repeat);
    ...
    void setup() {
        ...
        cont.pane(
                        t -> {
                            ...
                            t.update(
                                    () -> {
                                        if (Core.input.keyTap(Binding.chat)
                                                && Core.scene.getKeyboardFocus() == searchField
                                                && firstSchematic != null) {
                            ...
                            rebuildPane =
                                    () -> {
                                        ...
                                        for (Schematic s : schematics.all()) {
                                            // make sure *tags* fit
                                            if (selectedTags.any()
                                                    && !s.labels.containsAll(selectedTags))
                                                continue;
                                            // make sure search fits
                                            if (!search.isEmpty()
                                                    && !ignoreSymbols
                                                    .matcher(s.name().toLowerCase())
                                                    .replaceAll("")
                    ...
                    p.table(
                            ...
                            t -> {
                                ...
                                t.button(
                                        ...
                                        .disabled(
                                                b ->
                                                        Core.app.getClipboardText() == null
                                                                || !Core.app
                                                                .getClipboardText()
                                                                .startsWith(
                                                                        schematicBaseStart));
                            ...
                            resized(
                                    ...
                                    () -> {
                                        ...
                                        for (ContentType ctype : defaultContentIcons) {
                                            ...
                                            int i = 0;
                                            for (UnlockableContent u :
                                                    content.getBy(ctype).<UnlockableContent>as()) {
                                                if (!u.isHidden()
                                                        && u.unlockedNow()
                                                        && u.hasEmoji()
                                                        && !tags.contains(u.emoji())) {
                                                    t.button(
                    ...
                    rebuild[0] =
                            () -> {
                                ...
                                for (var tag : tags) {
                                    ...
                                    if (w + sum
                                            >= Core.graphics.getWidth()
                                            * (Core.graphics.isPortrait() ? 1f : 0.8f)) {
                                        p.add(current).row();
                                        current = new Table();
                                        current.left();
    ...
    public static class SchematicImage extends Image {
        ...
        @Override
        public void draw() {
            boolean checked = parent.parent instanceof Button && ((Button) parent.parent).isOver();
    ...
    public class SchematicInfoDialog extends BaseDialog {
        ...
        public void show(Schematic schem) {
            ...
            cont.add(
                            Core.bundle.format(
                                    "schematic.info", schem.width, schem.height, schem.tiles.size))
                        ...
                        for (ItemStack s : arr) {
                            ...
                            r.label(
                                            () -> {
                                                ...
                                                return (core.items.has(s.item, s.amount)
                                                        ? "[lightgray]"
                                                        : "[scarlet]")
                                                        + Math.min(core.items.get(s.item), s.amount)
                                                        + "[lightgray]/"
                                                        + s.amount;
                                            })
                                    .padLeft(2)
