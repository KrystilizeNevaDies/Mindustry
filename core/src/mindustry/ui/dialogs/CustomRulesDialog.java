...
public class CustomRulesDialog extends BaseDialog {
    ...
    private <T extends UnlockableContent> void showBanned(
            String title, ContentType type, ObjectSet<T> set, Boolf<T> pred) {
        ...
        rebuild[0] =
                () -> {
                    float previousScroll =
                            bd.cont.getChildren().isEmpty()
                                    ? 0f
                                    : ((ScrollPane) bd.cont.getChildren().first()).getScrollY();
                    bd.cont.clear();
                    bd.cont
                            .pane(
                                    t -> {
                                        ...
                                        int cols =
                                                mobile && Core.graphics.isPortrait()
                                                        ? 1
                                                        : mobile ? 2 : 3;
                                        int i = 0;
                            ...
                            .get()
                            .setScrollYForce(previousScroll);
                    bd.cont.row();
                    bd.cont
                            .button(
                                    "@add",
    ...
    void setup() {
        ...
        number(
                "@rules.wavespacing",
                false,
                f -> rules.waveSpacing = f * 60f,
                () -> rules.waveSpacing / 60f,
                () -> rules.waveTimer,
                1,
                Float.MAX_VALUE);
        ...
        if (experimental) {
            number(
                    ...
                    false,
                    f -> rules.initialWaveSpacing = f * 60f,
                    () -> rules.initialWaveSpacing / 60f,
                    () -> true,
                    0,
                    Float.MAX_VALUE);
        }
        number(
                "@rules.dropzoneradius",
                false,
                f -> rules.dropZoneRadius = f * tilesize,
                () -> rules.dropZoneRadius / tilesize,
                () -> true);

        title("@rules.title.resourcesbuilding");
        check(
                "@rules.infiniteresources",
                b -> rules.infiniteResources = b,
                () -> rules.infiniteResources);
        check(
                "@rules.onlydepositcore",
                b -> rules.onlyDepositCore = b,
                () -> rules.onlyDepositCore);
        check(
                "@rules.reactorexplosions",
                b -> rules.reactorExplosions = b,
                () -> rules.reactorExplosions);
        check("@rules.schematic", b -> rules.schematicsAllowed = b, () -> rules.schematicsAllowed);
        check(
                "@rules.coreincinerates",
                b -> rules.coreIncinerates = b,
                () -> rules.coreIncinerates);
        check(
                "@rules.cleanupdeadteams",
                b -> rules.cleanupDeadTeams = b,
                () -> rules.cleanupDeadTeams,
                () -> rules.pvp);
        check(
                "@rules.disableworldprocessors",
                b -> rules.disableWorldProcessors = b,
                () -> rules.disableWorldProcessors);
        number(
                "@rules.buildcostmultiplier",
                false,
                f -> rules.buildCostMultiplier = f,
                () -> rules.buildCostMultiplier,
                () -> !rules.infiniteResources);
        number(
                "@rules.buildspeedmultiplier",
                f -> rules.buildSpeedMultiplier = f,
                () -> rules.buildSpeedMultiplier,
                0.001f,
                50f);
        number(
                ...
                false,
                f -> rules.deconstructRefundMultiplier = f,
                () -> rules.deconstructRefundMultiplier,
                () -> !rules.infiniteResources);
        number(
                "@rules.blockhealthmultiplier",
                f -> rules.blockHealthMultiplier = f,
                () -> rules.blockHealthMultiplier);
        number(
                "@rules.blockdamagemultiplier",
                f -> rules.blockDamageMultiplier = f,
                () -> rules.blockDamageMultiplier);
                        ...
                        () ->
                                loadoutDialog.show(
                                        ...
                                        () ->
                                                rules.loadout
                                                        .clear()
                                                        .add(new ItemStack(Items.copper, 100)),
                                        () -> {
                                        },
                                        () -> {
                                        }))
        ...
        check(
                ...
                b -> rules.placeRangeCheck = b,
                () -> rules.placeRangeCheck);
        check(
                "@rules.polygoncoreprotection",
                b -> rules.polygonCoreProtection = b,
                () -> rules.polygonCoreProtection);
        number(
                "@rules.enemycorebuildradius",
                f -> rules.enemyCoreBuildRadius = f * tilesize,
                () -> Math.min(rules.enemyCoreBuildRadius / tilesize, 200),
                () -> !rules.polygonCoreProtection);
        ...
        if (experimental) {
            ...
            numberi(
                    "x",
                    x -> state.rules.limitX = x,
                    () -> state.rules.limitX,
                    () -> state.rules.limitMapArea,
                    0,
                    10000);
            numberi(
                    "y",
                    y -> state.rules.limitY = y,
                    () -> state.rules.limitY,
                    () -> state.rules.limitMapArea,
                    0,
                    10000);
            numberi(
                    "w",
                    w -> state.rules.limitWidth = w,
                    () -> state.rules.limitWidth,
                    () -> state.rules.limitMapArea,
                    0,
                    10000);
            numberi(
                    "h",
                    h -> state.rules.limitHeight = h,
        ...
        main.button(
                        ...
                        () -> ui.picker.show(rules.ambientLight, rules.ambientLight::set))
                .left()
                .width(250f)
                .row();
        ...
        main.table(
                        ...
                        t -> {
                            ...
                            for (Planet planet : new Planet[]{Planets.serpulo, Planets.erekir}) {
                                t.button(
                                                ...
                                                () -> {
                                                    ...
                                                    rules.hiddenBuildItems.addAll(
                                                            planet.hiddenItems);
                                                })
                                        .group(group)
                                        .checked(rules.env == planet.defaultEnv);
                            }
                            ...
                            t.button(
                                    ...
                                    .group(group)
                                    .checked(rules.hiddenBuildItems.size == 0);
                        })
                .left()
                .fill(false)
                .expand(false, false)
        ...
        for (Team team : Team.baseTeams) {
            ...
            main.collapser(
                            t -> {
                                ...
                                number(
                                        "@rules.blockhealthmultiplier",
                                        f -> teams.blockHealthMultiplier = f,
                                        () -> teams.blockHealthMultiplier);
                                number(
                                        "@rules.blockdamagemultiplier",
                                        f -> teams.blockDamageMultiplier = f,
                                ...
                                check(
                                        ...
                                        b -> teams.rtsAi = b,
                                        () -> teams.rtsAi,
                                        () -> team != rules.defaultTeam);
                                numberi(
                                        "@rules.rtsminsquadsize",
                                        f -> teams.rtsMinSquad = f,
                                        () -> teams.rtsMinSquad,
                                        () -> teams.rtsAi,
                                        0,
                                        100);
                                numberi(
                                        "@rules.rtsmaxsquadsize",
                                        f -> teams.rtsMaxSquad = f,
                                        () -> teams.rtsMaxSquad,
                                        () -> teams.rtsAi,
                                        1,
                                        1000);
                                number(
                                        ...
                                        f -> teams.rtsMinWeight = f,
                                        () -> teams.rtsMinWeight,
                                        () -> teams.rtsAi);

                                check(
                                        "@rules.infiniteresources",
                                        b -> teams.infiniteResources = b,
                                        () -> teams.infiniteResources);
                                number(
                                        "@rules.buildspeedmultiplier",
                                        f -> teams.buildSpeedMultiplier = f,
                                        () -> teams.buildSpeedMultiplier,
                                        0.001f,
                                        50f);

                                number(
                                        "@rules.unitdamagemultiplier",
                                        f -> teams.unitDamageMultiplier = f,
                                        () -> teams.unitDamageMultiplier);
                                number(
                                        "@rules.unitbuildspeedmultiplier",
                                        f -> teams.unitBuildSpeedMultiplier = f,
                                        () -> teams.unitBuildSpeedMultiplier,
                                        0.001f,
                                        50f);
                        ...
                        t -> {
                            ...
                            for (Team team : Team.baseTeams) {
                                t.button(
                                        ...
                                        .pad(1f)
                                        .checked(b -> prov.get() == team)
                                        .size(60f)
                                        .tooltip(team.localized())
                                        .with(i -> i.getStyle().imageUpColor = team.color);
                            }
                        })
                .padTop(0)
                .row();
        ...
        main.table(
                        t -> {
                            ...
                            t.field((prov.get()) + "", s -> cons.get(Strings.parseInt(s)))
                                    ...
                                    .valid(
                                            f ->
                                                    Strings.parseInt(f) >= min
                                                            && Strings.parseInt(f) <= max)
                                    .width(120f)
                                    .left();
                        })
                .padTop(0)
                                            ...
                                            a ->
                                                    a.setColor(
                                                            condition.get()
                                                                    ...
                                                                    : Color.gray));
                            t.field(
                                            (integer ? (int) prov.get() : prov.get()) + "",
                                            s -> cons.get(Strings.parseFloat(s)))
                                    .padRight(100f)
                                    .update(a -> a.setDisabled(!condition.get()))
                                    .valid(
                                            f ->
                                                    Strings.canParsePositiveFloat(f)
                                                            && Strings.parseFloat(f) >= min
                                                            && Strings.parseFloat(f) <= max)
                                    .width(120f)
                                    .left();
                        })
                .padTop(0);
                        ...
                        () -> {
                            ...
                            add.cont.pane(
                                    t -> {
                                        ...
                                        int i = 0;
                                        for (Weather weather :
                                                content.<Weather>getBy(ContentType.weather)) {
