...
public class JoinDialog extends BaseDialog {
        ...
        shown(
                () -> {
                    ...
                    if (!steam) {
                        Core.app.post(
                                () ->
                                        Core.settings.getBoolOnce(
                                                "joininfo", () -> ui.showInfo("@join.info")));
                    }
                });

        onResize(
                () -> {
                    // only refresh on resize when the minimum dimension is smaller than the maximum
                    // preferred
                    // width
                    // this means that refreshes on resize will only happen for small phones that
                    // need the
                    // list to fit in portrait mode
                    if (Math.min(Core.graphics.getWidth(), Core.graphics.getHeight())
                            / Scl.scl()
                            * 0.9f
                            < 500f) {
                        setup();
    ...
    void setupRemote() {
        ...
        for (Server server : servers) {
                                            ...
                                            () -> {
                                                if (!buttons[0].childrenPressed()) {
                                                    if (server.lastHost != null) {
                                                        Events.fire(
                                                                new ClientPreConnectEvent(
                                                                        server.lastHost));
                                                        safeConnect(
                                                                server.lastHost.address,
                                                                server.lastHost.port,
                                                                server.lastHost.version);
                                                    } else {
            ...
            inner.add("[accent]" + server.displayIP())
                    .left()
                    .padLeft(10f)
                    .wrap()
    ...
    void refreshServer(Server server) {
        ...
        server.content
                .label(
                        () ->
                                Core.bundle.get("server.refreshing")
                                        + Strings.animated(Time.time, 4, 11, "."))
                .padBottom(4);

        net.pingHost(
                server.ip,
                server.port,
                host -> setupServer(server, host),
                e -> {
                    server.content.clear();
    ...
    void buildServer(Host host, Table content) {
        ...
        if (host.version == -1) {
            versionString =
                    Core.bundle.format("server.version", Core.bundle.get("server.custombuild"), "");
        ...
        content.table(
                        ...
                        t -> {
                            ...
                            t.add(
                                            "[lightgray]"
                                                    + Core.bundle.format("save.map", host.mapname)
                                                    + "[lightgray] / "
                                                    + (host.modeName == null
                                                    ? host.mode.toString()
                                                    : host.modeName))
                            ...
                            if (host.ping > 0) {
                                t.add(Iconc.chartBar + " " + host.ping + "ms")
                                        .style(Styles.outlineLabel)
                                        .color(Pal.gray)
                                        .left();
                            }
                        ...
                        name -> {
                            ...
                            name.button(
                                            ...
                                            () -> {
                                                coll.toggle(false);
                                                Core.settings.put(
                                                        "collapsed-" + label, coll.isCollapsed());
                                            })
                                    .update(
                                            i ->
                                                    i.getStyle().imageUp =
    ...
    void refreshCommunity() {
        ...
        for (int i = 0; i < defaultServers.size; i++) {
            ServerGroup group =
                    defaultServers.get((i + defaultServers.size / 2) % defaultServers.size);
            ...
            for (String address : group.addresses) {
                ...
                int resport =
                        address.contains(":") ? Strings.parseInt(address.split(":")[1]) : port;
                net.pingHost(
                        resaddress,
                        resport,
                        res -> {
