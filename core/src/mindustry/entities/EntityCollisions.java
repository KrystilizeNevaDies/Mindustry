...
public class EntityCollisions {
    ...
    public void move(Hitboxc entity, float deltax, float deltay, SolidPred solidCheck) {
        ...
        while (Math.abs(deltax) > 0 || !movedx) {
            ...
            moveDelta(
                    entity,
                    Math.min(Math.abs(deltax), seg) * Mathf.sign(deltax),
                    0,
                    r,
                    true,
                    solidCheck);
        ...
        while (Math.abs(deltay) > 0 || !movedy) {
            ...
            moveDelta(
                    entity,
                    0,
                    Math.min(Math.abs(deltay), seg) * Mathf.sign(deltay),
                    r,
                    false,
                    solidCheck);
    ...
    public void moveDelta(
            Hitboxc entity, float deltax, float deltay, int r, boolean x, SolidPred solidCheck) {
        entity.hitboxTile(r1);
        entity.hitboxTile(r2);
        r1.x += deltax;
        r1.y += deltay;

        int tilex = Math.round((r1.x + r1.width / 2) / tilesize),
                tiley = Math.round((r1.y + r1.height / 2) / tilesize);
    ...
    public static boolean legsSolid(int x, int y) {
        ...
        return tile == null
                ...
                || (tile.floor().solid && tile.block() == Blocks.air);
    }
    ...
    private void checkCollide(Hitboxc a, Hitboxc b) {
        ...
        if (a != b && a.collides(b) && b.collides(a)) {
            ...
            boolean collide =
                    r1.overlaps(r2)
                            || collide(
                            r1.x, r1.y, r1.width, r1.height, vax, vay, r2.x, r2.y, r2.width,
                            r2.height, vbx, vby, l1);
            if (collide) {
