...
public class Lightning {
    ...
    public static void create(
            Team team, Color color, float damage, float x, float y, float targetAngle, int length) {
    ...
    public static void create(
            ...
            float targetAngle,
            int length) {
        createLightningInternal(
                bullet, lastSeed++, bullet.team, color, damage, x, y, targetAngle, length);
    }

    private static void createLightningInternal(
            @Nullable Bullet hitter,
            int seed,
            Team team,
            Color color,
            float damage,
        ...
        BulletType hitCreate =
                hitter == null || hitter.type.lightningType == null
            ...
            if (lines.size > 1) {
                ...
                World.raycastEach(
                        ...
                        (wx, wy) -> {
                            ...
                            if (tile != null
                                    && (tile.build != null && tile.build.isInsulated())
                                    && tile.team() != team) {
                                bhit = true;
                ...
                Units.nearbyEnemies(
                        ...
                        u -> {
                            if (!hit.contains(u.id())
                                    && (hitter == null
                                    || u.checkTarget(
                                    hitter.type.collidesAir,
                                    hitter.type.collidesGround))) {
                                entities.add(u);
                            }
                        });
            }
