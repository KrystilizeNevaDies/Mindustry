...
public class RegionPart extends DrawPart {
    ...
    public void draw(PartParams params) {
        ...
        for (int s = 0; s < len; s++) {
            ...
            Tmp.v1
                    .set((x + mx) * sign * Draw.xscl, (y + my) * Draw.yscl)
                    .rotateRadExact((params.rotation - 90) * Mathf.degRad);
            ...
            if (heat.found()) {
                Drawf.additive(
                        heat,
                        heatColor.write(Tmp.c1).a(heatProgress.getClamp(params) * heatColor.a),
                        rx,
                        ry,
                        rot,
                        turretShading ? turretHeatLayer : Draw.z() + heatLayerOffset);
            }
        ...
        if (children.size > 0) {
            for (int s = 0; s < len; s++) {
                ...
                childParam.set(
                        ...
                        params.x + Tmp.v1.x,
                        params.y + Tmp.v1.y,
                        i * sign + mr * sign + params.rotation);
    ...
    public void load(String name) {
        ...
        if (drawRegion) {
            if (mirror && turretShading) {
                regions =
                        new TextureRegion[]{
                                Core.atlas.find(realName + "-r"), Core.atlas.find(realName + "-l")
                        };
