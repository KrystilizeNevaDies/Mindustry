...
abstract class TankComp implements Posc, Flyingc, Hitboxc, Unitc, ElevationMovec {
    ...
    private transient float treadEffectTime, lastSlowdown = 1f;
    ...
    public void update() {
        ...
        if (walked && !headless && !inFogTo(player.team())) {
            ...
            if (treadEffectTime >= 6f && type.treadRects.length > 0) {
                ...
                for (int i : Mathf.signs) {
                    Tmp.v1
                            .set(xOffset * i, yOffset - treadRect.height / 2f / 4f)
                            .rotate(rotation - 90);

                    // TODO could fin for a while
                    Effect.floorDustAngle(
                            type.treadEffect, Tmp.v1.x + x, Tmp.v1.y + y, rotation + 180f);
                }
        ...
        for (int dx = -r; dx <= r; dx++) {
            for (int dy = -r; dy <= r; dy++) {
                ...
                if (type.crushDamage > 0
                        && walked
                        && t != null
                        && t.build != null
                        && t.build.team != team
                        // damage radius is 1 tile smaller to prevent it from just touching walls as
                        // it passes
                        && Math.max(Math.abs(dx), Math.abs(dy)) <= r - 1) {

                    t.build.damage(
                            team, type.crushDamage * Time.delta * t.block().crushDamageMultiplier);
                }
            }
        }
        ...
        lastSlowdown =
                Mathf.lerp(
                        1f,
                        type.crawlSlowdown,
                        Mathf.clamp((float) solids / total / type.crawlSlowdownFrac));
