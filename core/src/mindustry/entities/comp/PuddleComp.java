...
abstract class PuddleComp implements Posc, Puddlec, Drawc {
    ...
    private static Cons<Unit> unitCons =
            unit -> {
                    ...
                    if (rect.overlaps(rect2)) {
                        ...
                        if (unit.vel.len2() > 0.1f * 0.1f) {
                            Fx.ripple.at(
                                    unit.x,
                                    unit.y,
                                    unit.type.rippleScale,
                                    paramPuddle.liquid.color);
                        }
                    }
                }
            };
    ...
    transient float accepting,
            updateTime,
            lastRipple = Time.time + Mathf.random(40f),
            effectTime = Mathf.random(50f);
    float amount;
    Tile tile;
    Liquid liquid;
    ...
    public void update() {
        ...
        if (amount >= maxLiquid / 2f && updateTime <= 0f) {
            ...
            Units.nearby(
                    rect.setSize(Mathf.clamp(amount / (maxLiquid / 1.5f)) * 10f).setCenter(x, y),
                    unitCons);
