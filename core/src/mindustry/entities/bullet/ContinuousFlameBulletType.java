...
public class ContinuousFlameBulletType extends ContinuousBulletType {
     ...
     * Stored as an 'interleaved' array of values: LWPO1 LWPO2 LWPO3...
     */
    ...
    public void draw(Bullet b) {
        ...
        float realLength =
                (pierceCap <= 0 ? length : Damage.findPierceLength(b, pierceCap, length)) * mult;

        float sin = Mathf.sin(Time.time, oscScl, oscMag);

        for (int i = 0; i < colors.length; i++) {
            Draw.color(
                    colors[i].write(Tmp.c1).mul(0.9f).mul(1f + Mathf.absin(Time.time, 1f, 0.1f)));
            Drawf.flame(
                    b.x,
                    b.y,
                    divisions,
