...
public class Damage {
    ...
    public static void applySuppression(
            ...
            float reload,
            float maxDelay,
            float applyParticleChance,
            @Nullable Position source) {
        builds.clear();
        indexer.eachBlock(
                null,
                x,
                ...
                build -> {
                    ...
                    if (build.wasRecentlyHealed(60f * 12f) || build.block.suppressable) {
                        ...
                        // add prev check so ability spam doesn't lead to particle spam
                        // (essentially, recently
                        // suppressed blocks don't get new particles)
        ...
        // to prevent particle spam, the amount of particles is to remain constant (scales with
        // number
        // of buildings)
        ...
        for (var build : builds) {
            if (Mathf.chance(scaledChance)) {
                Time.run(
                        Mathf.random(maxDelay),
                        () -> {
                            Fx.regenSuppressSeek.at(
                                    build.x + Mathf.range(build.block.size * tilesize / 2f),
    ...
    public static void dynamicExplosion(
            ...
            Effect explosionFx) {
        dynamicExplosion(
                x, y, flammability, explosiveness, power, radius, damage, true, null, explosionFx);
    }
    ...
    public static void dynamicExplosion(
        ...
        if (damage) {
            for (int i = 0; i < Mathf.clamp(power / 700, 0, 8); i++) {
                ...
                Time.run(
                        i * 0.8f + Mathf.random(4f),
                        () ->
                                Lightning.create(
                                        Team.derelict,
                                        Pal.power,
                                        3 + Mathf.pow(power, 0.35f),
                                        x,
                                        y,
                                        Mathf.random(360f),
                                        length + Mathf.range(2)));
            }
                ...
                Time.run(
                        ...
                        () -> {
                            damage(
                                    ...
                                    x,
                                    y,
                                    Mathf.clamp(radius + explosiveness, 0, 50f)
                                            * ((f + 1f) / waves),
                                    explosiveness / 2f,
                                    false);
                            Fx.blockExplosionSmoke.at(
                                    x + Mathf.range(radius), y + Mathf.range(radius));
                        });
            }
        }
    ...
    public static @Nullable Building findAbsorber(
            Team team, float x1, float y1, float x2, float y2) {
        tmpBuilding = null;
        ...
        boolean found =
                World.raycast(
                        ...
                        World.toTile(y2),
                        (x, y) ->
                                (tmpBuilding = world.build(x, y)) != null
                                        && tmpBuilding.team != team
    ...
    public static float findLaserLength(Bullet b, float length) {
        ...
        boolean found =
                World.raycast(
                        b.tileX(),
                        b.tileY(),
                        World.toTile(b.x + vec.x),
                        World.toTile(b.y + vec.y),
                        (x, y) ->
                                (furthest = world.tile(x, y)) != null
                                        && furthest.team() != b.team
                                        && (furthest.build != null
                                        && furthest.build.absorbLasers()));

        return found && furthest != null
                ? Math.max(6f, b.dst(furthest.worldx(), furthest.worldy()))
                : length;
    }
    ...
    public static float findPierceLength(Bullet b, int pierceCap, float length) {
        ...
        World.raycast(
                ...
                (x, y) -> {
                    ...
                    if (build != null
                            && build.team != b.team
                            && b.checkUnderBuild(build, x * tilesize, y * tilesize)) {
        ...
        Units.nearbyEnemies(
                ...
                u -> {
                    ...
                    if (u.checkTarget(b.type.collidesAir, b.type.collidesGround)
                            ...
                            b.x, b.y, b.x + vec.x, b.y + vec.y, hitrect)) {
        ...
        return Math.min(
                distances.size < pierceCap || pierceCap < 0
                        ? length
                        : Math.max(6f, distances.get(pierceCap - 1)),
                maxDst);
    }

    /**
     * TODO Collides a bullet with blocks in a laser, taking into account absorption blocks.
     * Resulting length is stored in the bullet's fdata.
     */
    public static float collideLaser(
            Bullet b, float length, boolean large, boolean laser, int pierceCap) {
    ...
    public static void collideLine(
            Bullet hitter, Team team, Effect effect, float x, float y, float angle, float length) {
    ...
    public static void collideLine(
        ...
        Intc2 collider =
                (cx, cy) -> {
                    ...
                    boolean collide =
                            tile != null
                                    && hitter.checkUnderBuild(tile, cx * tilesize, cy * tilesize)
                    ...
                    if (hitter.damage > 0) {
                        ...
                        if (collide && hitter.type.testCollision(hitter, tile)) {
                            hitter.type.hitTile(
                                    hitter, tile, cx * tilesize, cy * tilesize, health, false);
                        }
                    }
                };
                    ...
                    (cx, cy) -> {
                        ...
                        for (Point2 p : Geometry.d4) {
                            ...
                            if (other != null
                                    && (large
                                    || Intersector.intersectSegmentRectangle(
                                    seg1, seg2, other.getBounds(Tmp.r1)))) {
    ...
    public static Healthc linecast(Bullet hitter, float x, float y, float angle, float length) {
        ...
        if (tmpBuilding != null && tmpUnit != null) {
            if (Mathf.dst2(x, y, tmpBuilding.getX(), tmpBuilding.getY())
                    <= Mathf.dst2(x, y, tmpUnit.getX(), tmpUnit.getY())) {
                return tmpBuilding;
            }
    ...
    public static void damage(
            Team team, float x, float y, float radius, float damage, boolean air, boolean ground) {
     ...
     */
    public static void damage(
            Team team, float x, float y, float radius, float damage, boolean complete) {
    ...
    public static void damage(
        ...
        Cons<Unit> cons =
                entity -> {
                    if (entity.team == team
                            || !entity.hittable()
                            || !entity.within(x, y, radius + (scaled ? entity.hitSize / 2f : 0f))
                            || (entity.isFlying() && !air)
                            || (entity.isGrounded() && !ground)) {
                        return;
                    }
                    ...
                    float amount =
                            calculateDamage(
                                    scaled
                                            ? Math.max(
                                            0, entity.dst(x, y) - entity.type.hitSize / 2)
                                            : entity.dst(x, y),
                                    radius,
                                    damage);
        ...
        if (ground) {
            if (!complete) {
                tileDamage(
                        ...
                        World.toTile(y),
                        radius / tilesize,
                        damage * (source == null ? 1f : source.type.buildingDamageMultiplier),
                        source);
            } else {
    ...
    public static void tileDamage(
            Team team, int x, int y, float baseRadius, float damage, @Nullable Bullet source) {
        ...
        Core.app.post(
                () -> {
                    ...
                    // which means
                    // much less damage than a nearby explosion.
                    // this needs to be compensated
                    if (in != null && in.team != team && in.block.size > 1 && in.health > damage) {
                        // deal the damage of an entire side, to be equivalent with maximum
                        // 'standard' damage
                    ...
                    for (int i = 0; i <= rays; i++) {
                        ...
                        while (startX != endX || startY != endY) {
                            ...
                            if (build != null && build.team != team) {
                                ...
                                float mult =
                                        (1f - (Mathf.dst2(startX, startY, x, y) / rad2) + edgeScale)
                                                / (1f + edgeScale);
    ...
    private static void completeDamage(Team team, float x, float y, float radius, float damage) {
        ...
        for (int dx = -trad; dx <= trad; dx++) {
            for (int dy = -trad; dy <= trad; dy++) {
                Tile tile =
                        world.tile(Math.round(x / tilesize) + dx, Math.round(y / tilesize) + dy);
                if (tile != null
                        && tile.build != null
                        && (team == null || team.isEnemy(tile.team()))
