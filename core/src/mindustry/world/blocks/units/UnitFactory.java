...
public class UnitFactory extends UnitBlock {
    ...
    public UnitFactory(String name) {
        ...
        config(
                Integer.class,
                (UnitFactoryBuild tile, Integer i) -> {
        ...
        config(
                UnitType.class,
                (UnitFactoryBuild tile, UnitType val) -> {
                    if (!configurable) return;
        ...
        consume(
                new ConsumeItemDynamic(
                        (UnitFactoryBuild e) ->
                                e.currentPlan != -1
                                        ? plans.get(Math.min(e.currentPlan, plans.size - 1))
                                        .requirements
                                        : ItemStack.empty));
    }
                ...
                (UnitFactoryBuild e) ->
                        new Bar(
                                ...
                                () ->
                                        e.unit() == null
                                                ? 0f
                                                : (float) e.team.data().countType(e.unit())
                                                / Units.getCap(e.team)));
    }
    ...
    public class UnitFactoryBuild extends UnitBuild {
        ...
        public void buildConfiguration(Table table) {
            Seq<UnitType> units =
                    Seq.with(plans).map(u -> u.unit).filter(u -> u.unlockedNow() && !u.isBanned());
        ...
        public boolean acceptItem(Building source, Item item) {
            return currentPlan != -1
                    ...
                    && Structs.contains(
                    plans.get(currentPlan).requirements, stack -> stack.item == item);
        }
