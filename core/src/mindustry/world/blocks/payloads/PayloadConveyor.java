...
public class PayloadConveyor extends Block {
    ...
    public class PayloadConveyorBuild extends Building {
        ...
        public void onProximityUpdate() {
            ...
            Building accept =
                    nearby(
                            Geometry.d4(rotation).x * (size / 2 + 1),
                            Geometry.d4(rotation).y * (size / 2 + 1));
            ...
            int ntrns = 1 + size / 2;
            Tile next =
                    tile.nearby(Geometry.d4(rotation).x * ntrns, Geometry.d4(rotation).y * ntrns);
            blocked =
                    (next != null
                            && next.solid()
                            && !(next.block().outputsPayload
                            || next.block().acceptsPayload))
            ...
            if (curStep > step) {
                ...
                if (valid && stepAccepted != curStep && item != null) {
                    if (next != null) {
                        ...
                        // TODO add self to queue of next conveyor, then check if this conveyor was
                        // selected
                        // next frame - selection happens deterministically
        ...
        public void draw() {
            ...
            float trnext = fract() * size * tilesize,
                    trprev = size * tilesize * (fract() - 1),
                    rot = rotdeg();

            TextureRegion clipped =
                    clipRegion(
                            tile.getHitbox(Tmp.r1),
                            tile.getHitbox(Tmp.r2).move(trnext, 0),
                            topRegion);
            ...
            Tmp.v1
                    .set(
                            (s - clipped.width * Draw.scl) + clipped.width / 2f * Draw.scl - s / 2f,
                            s - clipped.height * Draw.scl + clipped.height / 2f * Draw.scl - s / 2f)
                    .rotate(rot);
            ...
            clipped =
                    clipRegion(
                            tile.getHitbox(Tmp.r1),
                            tile.getHitbox(Tmp.r2).move(trprev, 0),
                            topRegion);

            // prev
            Tmp.v1
                    .set(
                            -s / 2f + clipped.width / 2f * Draw.scl,
                            -s / 2f + clipped.height / 2f * Draw.scl)
                    .rotate(rot);
            Draw.rect(clipped, x + Tmp.v1.x, y + Tmp.v1.y, rot);
        ...
        protected TextureRegion clipRegion(Rect bounds, Rect sprite, TextureRegion region) {
            ...
            if (overlaps) {
                ...
                float newW = (over.width / sprite.width) * w,
                        newH = (over.height / sprite.height) * h;
