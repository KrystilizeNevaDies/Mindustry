...
public class WobbleProp extends Prop {
    ...
    public void drawBase(Tile tile) {
        ...
        Draw.rectv(
                ...
                vec ->
                        vec.add(
                                Mathf.sin(vec.y * 3 + Time.time, wscl, wmag)
                                        + Mathf.sin(
                                        vec.x * 3 - Time.time, 70 * wtscl, 0.8f * wmag2),
                                Mathf.cos(vec.x * 3 + Time.time + 8, wscl + 6f, wmag * 1.1f)
                                        + Mathf.sin(
                                        vec.y * 3 - Time.time, 50 * wtscl, 0.2f * wmag2)));
    }
}
