...
public class Seaweed extends Prop {
    ...
    public void drawBase(Tile tile) {
        ...
        float x = tile.worldx(),
                ...
                rotscl = 0.5f,
                rot =
                        Mathf.randomSeedRange(tile.pos(), 20f)
                                - 45
                                + Mathf.sin(Time.time + x, 50f * rotscl, 0.5f * rotmag)
                                + Mathf.sin(Time.time - y, 65f * rotscl, 0.9f * rotmag)
                                + Mathf.sin(Time.time + y - x, 85f * rotscl, 0.9f * rotmag),
