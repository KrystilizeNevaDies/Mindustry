...
public class Floor extends Block {
        ...
        for (int x = 0; x < edge.width; x++) {
            for (int y = 0; y < edge.height; y++) {
                result.set(
                        x,
                        y,
                        Color.muli(edge.get(x, y), image.get(x % image.width, y % image.height)));
            }
        }
    ...
    public void drawBase(Tile tile) {
        ...
        Draw.rect(
                variantRegions[
                        Mathf.randomSeed(tile.pos(), 0, Math.max(0, variantRegions.length - 1))],
                tile.worldx(),
                tile.worldy());
    ...
    /**
     * @return true if this floor should be updated in the render loop, e.g. for effects. Do NOT
     * overuse this!
     */
    ...
    public void drawNonLayer(Tile tile, CacheLayer layer) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (other != null
                    && other.floor().cacheLayer == layer
                    && other.floor().edges() != null) {
    ...
    protected void drawEdges(Tile tile) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (other != null
                    ...
                    && other.floor().cacheLayer == realCache
                    && other.floor().edges() != null) {
    ...
    public int realBlendId(Tile tile) {
        if (tile.floor().isLiquid
                && !tile.overlay().isAir()
                && !(tile.overlay() instanceof OreBlock)) {
