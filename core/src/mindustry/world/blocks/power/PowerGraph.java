...
public class PowerGraph {
    ...
    public void distributePower(float needed, float produced, boolean charged) {
        // distribute even if not needed. this is because some might be requiring power but not
        // using
        // it; it updates consumers
        float coverage =
                Mathf.zero(needed)
                        && Mathf.zero(produced)
                        && !charged
                        && Mathf.zero(lastPowerStored)
                        ? 0f
                        : Mathf.zero(needed) ? 1f : Math.min(1, produced / needed);
        var items = consumers.items;
        for (int i = 0; i < consumers.size; i++) {
            var consumer = items[i];
            ...
            if (cons.buffered) {
                if (!Mathf.zero(cons.capacity)) {
                    // Add an equal percentage of power to all buffers, based on the global power
                    // coverage in
                    // this graph
                    float maximumRate = cons.requestedPower(consumer) * coverage * consumer.delta();
                    consumer.power.status =
                            Mathf.clamp(consumer.power.status + maximumRate / cons.capacity);
                }
            } else {
                ...
                if (otherConsumersAreValid(consumer, cons)) {
                    ...
                    consumer.power.status =
                            Math.min(1, produced / (needed + cons.usage * consumer.delta()));
                    // just in case
    ...
    /**
     * Note that this does not actually remove the building from the graph; it creates *new* graphs
     * that contain the correct buildings. Doing this invalidates the graph.
     */
    public void remove(Building tile) {
        ...
        for (Building other : tile.getPowerConnections(outArray1)) {
            ...
            while (queue.size > 0) {
                ...
                for (Building next : child.getPowerConnections(outArray2)) {
                    // make sure it hasn't looped back, and that the new graph being assigned hasn't
                    // already
                    // been assigned
            ...
            // update the graph once so direct consumers without any connected producer lose their
            // power
            graph.update();
        }
