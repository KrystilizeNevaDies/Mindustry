...
public class RegenProjector extends Block {
    ...
    public void drawPlace(int x, int y, int rotation, boolean valid) {
        ...
        indexer.eachBlock(
                Vars.player.team(),
                Tmp.r1.setCentered(x, y, range * tilesize),
                b -> true,
                t -> {
    ...
    public class RegenProjectorBuild extends Building {
        ...
        public void updateTargets() {
            ...
            taken.clear();
            indexer.eachBlock(
                    team, Tmp.r1.setCentered(x, y, range * tilesize), b -> true, targets::add);
        }
        ...
        public void updateTile() {
            ...
            if (efficiency > 0) {
                ...
                float healAmount =
                        Mathf.lerp(1f, optionalMultiplier, optionalEfficiency) * healPercent;
                ...
                for (var build : targets) {
                    ...
                    mendMap.put(
                            ...
                            Math.min(
                                    Math.max(
                                            value,
                                            healAmount * edelta() * build.block.health / 100f),
                                    build.block.health - build.health));

                    if (value <= 0
                            && Mathf.chanceDelta(
                            effectChance * build.block.size * build.block.size)) {
                        effect.at(
                                build.x + Mathf.range(build.block.size * tilesize / 2f - 1f),
                                build.y + Mathf.range(build.block.size * tilesize / 2f - 1f));
                    }
                }
            }
