...
public class DrawFlame extends DrawBlock {
    ...
    public void draw(Building build) {
        if (build.warmup() > 0f && flameColor.a > 0.001f) {
            ...
            Draw.alpha(
                    ((1f - g) + Mathf.absin(Time.time, 8f, g) + Mathf.random(r) - r)
                            * build.warmup());
            ...
            Fill.circle(
                    build.x,
                    build.y,
                    flameRadius + Mathf.absin(Time.time, flameRadiusScl, flameRadiusMag) + cr);
    ...
    public void drawLight(Building build) {
        Drawf.light(
                build.x,
                build.y,
                (lightRadius + Mathf.absin(lightSinScl, lightSinMag))
                        * build.warmup()
                        * build.block.size,
                flameColor,
                lightAlpha);
