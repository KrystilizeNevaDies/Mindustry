...
public class DrawTurret extends DrawBlock {
    ...
    public void getRegionsToOutline(Block block, Seq<TextureRegion> out) {
        ...
        if (block.region.found()
                ...
                && block.getGeneratedIcons()[block.outlinedIcon].equals(block.region))) {
    ...
    public void draw(Building build) {
        ...
        if (parts.size > 0) {
            if (outline.found()) {
                ...
                Draw.rect(
                        outline,
                        build.x + tb.recoilOffset.x,
                        build.y + tb.recoilOffset.y,
                        tb.drawrot());
                Draw.z(Layer.turret);
            }
    ...
    public TextureRegion[] icons(Block block) {
        return top.found()
                ? new TextureRegion[]{base, preview, top}
                : new TextureRegion[]{base, preview};
    }
}
