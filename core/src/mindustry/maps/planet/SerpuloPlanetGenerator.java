...
public class SerpuloPlanetGenerator extends PlanetGenerator {
    ...
    float rawHeight(Vec3 position) {
        ...
        return (Mathf.pow(
                Simplex.noise3d(
                        seed, 7, 0.5f, 1f / 3f, position.x, position.y, position.z),
                2.3f)
                + waterOffset)
                / (1f + waterOffset);
    }
    ...
    Block getBlock(Vec3 position) {
        ...
        float tnoise =
                Simplex.noise3d(seed, 7, 0.56, 1f / 3f, position.x, position.y + 999f, position.z);
        temp = Mathf.lerp(temp, tnoise, 0.5f);
        height *= 1.2f;
        height = Mathf.clamp(height);

        float tar =
                Simplex.noise3d(seed, 4, 0.55f, 1f / 2f, position.x, position.y + 999f, position.z)
                        * 0.3f
                        + Tmp.v31.dst(0, 0, 1f) * 0.2f;

        Block res =
                arr[Mathf.clamp((int) (temp * arr.length), 0, arr[0].length - 1)][
    ...
    protected float noise(
            float x, float y, double octaves, double falloff, double scl, double mag) {
    ...
    protected void generate() {
        ...
        class Room {
            ...
            void connect(Room to) {
                ...
                if (alt) {
                    midpoint.add(
                            Tmp.v2
                                    .set(1, 0f)
                                    .setAngle(
                                            Angles.angle(to.x, to.y, x, y)
                                                    + 90f * (rand.chance(0.5) ? 1f : -1f))
                ...
                midpoint.sub(width / 2f, height / 2f)
            ...
            void joinLiquid(int x1, int y1, int x2, int y2) {
                ...
                var path =
                        pathfind(
                                ...
                                y2,
                                tile ->
                                        (tile.solid() || !tile.floor().isLiquid ? 70f : 0f)
                                                + noise(tile.x, tile.y, 2, 0.4f, 1f / nscl) * 500,
                                Astar.manhattan);
                path.each(
                        t -> {
        ...
        // check positions on the map to place the player spawn. this needs to be in the corner of
        // the
        // map
        Room spawn = null;
        ...
        pass(
                (x, y) -> {
                    ...
                    float value =
                            Ridged.noise3d(2, v.x, v.y, v.z, 1, 1f / 55f) + rr - rawHeight(v) * 0f;
                    float rrscl = rr * 44 - 2;

                    if (value > 0.17f && !Mathf.within(x, y, fspawn.x, fspawn.y, 12 + rrscl)) {
                        boolean deep =
                                value > 0.17f + 0.1f
                                        && !Mathf.within(x, y, fspawn.x, fspawn.y, 15 + rrscl);
                        ...
                        if (!(floor == Blocks.ice
                            ...
                            floor =
                                    spore
                                            ...
                                            : (deep
                                            ? Blocks.water
                                            : (floor == Blocks.sand || floor == Blocks.salt
                                            ? Blocks.sandWater
                                            : Blocks.darksandWater));
                        }
                    }
            ...
            pass(
                    (x, y) -> {
                        if (floor.asFloor().isLiquid
                                && !floor.asFloor().isDeep()
                                && !floor.asFloor().shallow) {
        ...
        if (Simplex.noise3d(seed, 2, 0.5, scl, sector.tile.v.x, sector.tile.v.y, sector.tile.v.z)
                * nmag
                + poles
                > 0.25f * addscl) {
        ...
        pass(
                (x, y) -> {
                    ...
                    for (int i = ores.size - 1; i >= 0; i--) {
                        ...
                        if (Math.abs(0.5f - noise(offsetX, offsetY + i * 999, 2, 0.7, (40 + i * 2)))
                                > 0.22f + i * 0.01
                                && Math.abs(
                                0.5f
                                        - noise(
        ...
        pass(
                (x, y) -> {
                    ...
                    if (floor == Blocks.hotrock) {
                        if (Math.abs(0.5f - noise(x - 90, y, 4, 0.8, 80)) > 0.035) {
                            ...
                            for (Point2 p : Geometry.d4) {
                                ...
                                if (other == null
                                        || (other.floor() != Blocks.hotrock
                                        && other.floor() != Blocks.magmarock)) {
                                    all = false;
                                }
                            }
                            if (all) {
                        ...
                        if (noise > 0.67f
                                && !roomseq.contains(e -> Mathf.within(x, y, e.x, e.y, 14))) {
                            if (noise > 0.72f) {
                                floor =
                                        noise > 0.78f
                                                ? Blocks.taintedWater
                    ...
                    if (rand.chance(0.0075)) {
                        ...
                        if (any
                                && ((block == Blocks.snowWall || block == Blocks.iceWall)
                                || (all
                                && block == Blocks.air
                                && floor == Blocks.snow
                                && rand.chance(0.03)))) {
        ...
        if (ruinCount > 0) {
            ...
            for (int i = 0; i < ints.size && placed < ruinCount; i++) {
                ...
                if (part != null
                    ...
                    int debrisRadius =
                            Math.max(part.schematic.width, part.schematic.height) / 2 + 3;
                    Geometry.circle(
                            x,
                            y,
                            tiles.width,
                            ...
                            (cx, cy) -> {
                                ...
                                if (other.build != null && other.isCenter()) {
                                    if (other.team() == Team.derelict
                                            && rand.chance(removeChance)) {
                                        other.remove();
                                    } else if (rand.chance(0.5)) {
                                        other.build.health =
                                                other.build.health
                                                        - rand.random(other.build.health * 0.9f);
                                    }
                                }
                            });
                }
        ...
        if (sector.hasEnemyBase()) {
            basegen.generate(
                    tiles,
                    enemies.map(r -> tiles.getn(r.x, r.y)),
                    tiles.get(spawn.x, spawn.y),
        ...
        state.rules.waveSpacing =
                Mathf.lerp(60 * 65 * 2, 60f * 60f * 1f, Math.max(difficulty - waveTimeDec, 0f));
        ...
        state.rules.spawns =
                Waves.generate(
                        ...
                        state.rules.attackMode && spawner.countGroundSpawns() == 0,
                        naval);
    }
