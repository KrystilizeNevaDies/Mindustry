...
public class ErekirPlanetGenerator extends PlanetGenerator {
    ...
    float rawTemp(Vec3 position) {
        return position.dst(0, 0, 1) * 2.2f
                - Simplex.noise3d(
                seed,
                8,
                0.54f,
    ...
    protected void generate() {
        ...
        int spawnX = (int) (trns.x + width / 2f),
        ...
        brush(
                pathfind(
                        ...
                        tile ->
                                (tile.solid() ? 300f : 0f)
                                        + maxd
                                        - tile.dst(width / 2f, height / 2f) / 10f,
                        Astar.manhattan),
                9);
        erase(endX, endY, 15);
        ...
        pass(
                (x, y) -> {
                    // rough rhyolite
                    if (noise(x, y + 600 + x, 5, 0.86f, 60f, 1f) < 0.41f
                    ...
                    if (floor == Blocks.slag
                            && Mathf.within(
                            x, y, spawnX, spawnY, 30f + noise(x, y, 2, 0.8f, 9f, 15f))) {
                        floor = Blocks.yellowStonePlates;
                    }

                    if ((floor == Blocks.arkyciteFloor || floor == Blocks.arkyicStone)
                            && block.isStatic()) {
                    ...
                    if (floor == Blocks.yellowStonePlates
                            && noise(x + 78 + y, y, 3, 0.8f, 6f, 1f) > 0.44f) {
                        floor = Blocks.yellowStone;
                    }

                    if (floor == Blocks.redStone
                            && noise(x + 78 - y, y, 4, 0.73f, 19f, 1f) > 0.63f) {
        ...
        pass(
                (x, y) -> {
                    if (block != Blocks.air) {
                        if (nearAir(x, y)) {
                            if (block == Blocks.carbonWall
                                    && noise(x + 78, y, 4, 0.7f, 33f, 1f) > 0.52f) {
                                block = Blocks.graphiticWall;
                            } else if (block != Blocks.carbonWall
                                    && noise(x + 782, y, 4, 0.8f, 38f, 1f) > 0.665f) {
                        ...
                        if (noise(x + 999, y + 600 - x, 4, 0.63f, 45f, 1f) < 0.27f
                                && floor == Blocks.crystallineStone) {
                            ore = Blocks.oreCrystalThorium;
                        }
                    }

                    if (noise(x + 999, y + 600 - x, 5, 0.8f, 45f, 1f) < 0.44f
                            && floor == Blocks.crystallineStone) {
                        floor = Blocks.crystalFloor;
                    }

                    if (block == Blocks.air
                            && (floor == Blocks.crystallineStone || floor == Blocks.crystalFloor)
                            && rand.chance(0.09)
                            && nearWall(x, y)
                            && !near(x, y, 4, Blocks.crystalCluster)
                            && !near(x, y, 4, Blocks.vibrantCrystalCluster)) {
                        block =
                                floor == Blocks.crystalFloor
                                        ? Blocks.vibrantCrystalCluster
                                        : Blocks.crystalCluster;
                        ore = Blocks.air;
                    }

                    if (block == Blocks.arkyicWall
                            && rand.chance(0.23)
                            && nearAir(x, y)
                            && !near(x, y, 3, Blocks.crystalOrbs)) {
                    ...
                    if (block == Blocks.regolithWall
                            && rand.chance(0.3)
                            && nearAir(x, y)
                            && !near(x, y, 3, Blocks.crystalBlocks)) {
        ...
        pass(
                (x, y) -> {
                    if (ore.asFloor().wallOre
                            || block.itemDrop != null
                            || (block == Blocks.air && ore != Blocks.air)) {
            ...
            if ((floor == Blocks.rhyolite || floor == Blocks.roughRhyolite) && rand.chance(0.002)) {
                ...
                for (int x = -radius; x <= radius; x++) {
                    for (int y = -radius; y <= radius; y++) {
                        ...
                        if (other == null
                                || (other.floor() != Blocks.rhyolite
                                && other.floor() != Blocks.roughRhyolite)
                                || other.block().solid) {
                            continue outer;
                        }
                    }
        ...
        while (ventCount < minVents && iterations++ < maxIterations) {
            ...
            for (Tile tile : tiles) {
                if (rand.chance(0.00018 * (1 + iterations))
                        && !Mathf.within(tile.x, tile.y, spawnX, spawnY, 5f)) {
                    // skip crystals, but only when directly on them
                    if (tile.floor() == Blocks.crystallineStone
                    ...
                    for (int x = -radius; x <= radius; x++) {
                        for (int y = -radius; y <= radius; y++) {
                            ...
                            if (other == null
                                    || other.block().solid
                                    || other.floor().attributes.get(Attribute.steam) != 0
                                    || other.floor() == Blocks.slag
                                    || other.floor() == Blocks.arkyciteFloor) {
                                continue outer;
                            }
                        }
                    }
                    ...
                    if (tile.floor() == Blocks.beryllicStone
                            || tile.floor() == Blocks.arkyicStone) {
                        floor = secondFloor = Blocks.arkyicStone;
                        vent = Blocks.arkyicVent;
                    } else if (tile.floor() == Blocks.yellowStone
                            || tile.floor() == Blocks.yellowStonePlates
                            || tile.floor() == Blocks.regolith) {
                        floor = Blocks.yellowStone;
                        secondFloor = Blocks.yellowStonePlates;
                        vent = Blocks.yellowStoneVent;
                    } else if (tile.floor() == Blocks.redStone
                            || tile.floor() == Blocks.denseRedStone) {
                        floor = Blocks.denseRedStone;
                    ...
                    for (int cx = -crad; cx <= crad; cx++) {
                        for (int cy = -crad; cy <= crad; cy++) {
                            ...
                            if (cx * cx + rcy * rcy
                                ...
                                if (dest != null
                                        && dest.floor().attributes.get(Attribute.steam) == 0
                                        && dest.floor() != Blocks.roughRhyolite
                                        && dest.floor() != Blocks.arkyciteFloor
                                        && dest.floor() != Blocks.slag) {
