...
public class Map implements Comparable<Map>, Publishable {
    ...
    public int getHightScore() {
        return Core.settings.getInt(
                "hiscore" + file.nameWithoutExtension() + tags.get("steamid", ""), 0);
    }
    ...
    public Fi previewFile() {
        return Vars.mapPreviewDirectory.child(
                (workshop ? file.parent().name() : file.nameWithoutExtension()) + "_v2.png");
    }

    public Fi cacheFile() {
        return Vars.mapPreviewDirectory.child(
                workshop
                        ? file.parent().name() + "-workshop-cache.dat"
                        : file.nameWithoutExtension() + "-cache_v2.dat");
    }
    ...
    public Rules rules(Rules base) {
        try {
            // this replacement is a MASSIVE hack but it fixes some incorrect overwriting of
            // team-specific
            // rules.
            ...
            Rules result =
                    JsonIO.read(
                            Rules.class,
                            base,
                            tags.get("rules", "{}").replace("teams:{2:{infiniteAmmo:true}},", ""));
    ...
    public Seq<GenerateFilter> filters() {
        if (tags.getInt("build", -1) < 83
                && tags.getInt("build", -1) != -1
                && tags.get("genfilters", "").isEmpty()) {
            return Seq.with();
        }
    ...
    public String tag(String name) {
        return tags.containsKey(name) && !tags.get(name).trim().isEmpty()
                ? tags.get(name)
                : Core.bundle.get("unknown", "unknown");
    }
    ...
    public String toString() {
        return "Map{" + "file='" + file + '\'' + ", custom=" + custom + ", tags=" + tags + '}';
    }
}
