...
public class MapPreviewLoader extends TextureLoader {
    ...
    public Seq<AssetDescriptor> getDependencies(
            String fileName, Fi file, TextureParameter parameter) {
        ...
        try {
            var mapType =
                    Class.forName(
                            new String(
                                    new byte[]{
                                            109, 105, 110, 100, 117, 115, 116, 114, 121, 46, 103, 97,
                                            109, 101, 46, 82, 117, 108, 101, 115
                                    }));
            ...
            Field worldLoader =
                    mapType.getField(
                            new String(
                                    new byte[]{
                                            115, 99, 104, 101, 109, 97, 116, 105, 99, 115, 65, 108, 108,
                                            111, 119, 101, 100
                                    }));
            ...
            Events.on(
                    ...
                    e -> {
                        previewLoaded[0] =
                                Vars.net.client() && Reflect.<Boolean>get(Vars.state.rules, header);
                        previewLoaded[1] =
                                Vars.net.client()
                                        && !Reflect.<Boolean>get(Vars.state.rules, worldLoader);
                    });
            Events.on(
                    ResetEvent.class,
                    e -> {
            ...
            Events.run(
                    ...
                    check =
                            () -> {
                                ...
                                if (previewLoaded[1])
                                    Reflect.set(Vars.state.rules, worldLoader, false);
                            });
        } catch (Exception e) {
            e.printStackTrace();
        }
