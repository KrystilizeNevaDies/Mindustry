...
public class Maps {
    ...
    /**
     * Set the provider for the map(s) to be played on. Will override the default shuffle mode
     * setting.
     */
    ...
    public Maps() {
        ...
        if (Core.assets != null) {
            ((CustomLoader) Core.assets.getLoader(ContentLoader.class)).loaded =
                    this::createAllPreviews;
        }
    }

    /**
     * Loads a map from the map folder and returns it. Should only be used for zone maps. Does not
    ...
    /**
     * Save a custom map to the directory. This updates all values and stored data necessary. The
     * tags are copied to prevent mutation later.
     */
    ...
    /**
     * Attempts to run the following code; catches any errors and attempts to display them in a
     * readable way.
     */
    ...
    public Seq<SpawnGroup> readWaves(String str) {
        return str == null
                ...
                : str.equals("[]")
                ? new Seq<>()
                : Seq.with(JsonIO.json.fromJson(SpawnGroup[].class, str));
    }
    ...
    public void loadPreviews() {
        ...
        for (Map map : maps) {
            ...
            if (map.previewFile().exists()) {
                ...
                Core.assets.load(
                        new AssetDescriptor<>(
                                map.previewFile().path() + "." + mapExtension,
                                Texture.class,
                                new MapPreviewParameter(map)))
                        .loaded =
                        t -> map.texture = t;
    ...
    private void writeCache(Map map) throws IOException {
        try (DataOutputStream stream =
                     new DataOutputStream(map.cacheFile().write(false, Streams.defaultBufferSize))) {
            stream.write(0);
    ...
    private void readCache(Map map) throws IOException {
        try (DataInputStream stream =
                     new DataInputStream(map.cacheFile().read(Streams.defaultBufferSize))) {
            stream.read(); // version
        ...
        custom(
                (mode, prev) ->
                        next(
                                mode,
                                prev,
                                Vars.maps.customMaps().isEmpty()
                                        ? Vars.maps.defaultMaps()
                                        : Vars.maps.customMaps())),
