...
public abstract class GenerateFilter implements Cloneable {
        ...
        if (isBuffered()) {
            ...
            for (int i = 0; i < tiles.width * tiles.height; i++) {
                ...
                Block block = Vars.content.block(PackTile.block(b)),
                        floor = Vars.content.block(PackTile.floor(b)),
                        overlay = Vars.content.block(PackTile.overlay(b));

                tile.setFloor(floor.asFloor());
                tile.setOverlay(
                        !floor.asFloor().hasSurface()
                                && overlay.asFloor().needsSurface
                                && overlay instanceof OreBlock
                                ? Blocks.air
                                : overlay);
            ...
            for (Tile tile : tiles) {
                ...
                tile.setOverlay(
                        !in.floor.asFloor().hasSurface()
                                && in.overlay.asFloor().needsSurface
                                && in.overlay instanceof OreBlock
                                ? Blocks.air
                                : in.overlay);
    ...
    /**
     * @return whether this filter can *only* be used while generating the map, e.g. is not
     * undoable.
     */
    ...
    protected float noise(
            GenerateInput in, float scl, float mag, float octaves, float persistence) {
        return Simplex.noise2d(seed, octaves, persistence, 1f / scl, in.x, in.y) * mag;
    }

    protected float noise(
            float x, float y, float scl, float mag, float octaves, float persistence) {
    ...
    public static class GenerateInput {
        ...
        Tile tile(float x, float y) {
            return buffer.get(
                    Mathf.clamp((int) x, 0, width - 1), Mathf.clamp((int) y, 0, height - 1));
        }
