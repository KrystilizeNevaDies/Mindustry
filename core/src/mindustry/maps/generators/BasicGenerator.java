...
public abstract class BasicGenerator implements WorldGenerator {
        ...
        tiles.each(
                (x, y) -> {
                    ...
                    floors[x + y * width] =
                            ints1.get(
                                    Mathf.clamp(
                                            (int) (ints1.size * percentile), 0, ints1.size - 1));
                    blocks[x + y * width] =
                            ints2.get(
                                    Mathf.clamp(
                                            (int) (ints2.size * percentile), 0, ints2.size - 1));
                });
    ...
    public void ore(Block dest, Block src, float i, float thresh) {
        pass(
                (x, y) -> {
                    ...
                    if (Math.abs(0.5f - noise(x, y + i * 999, 2, 0.7, (40 + i * 2)))
                            > 0.26f * thresh
                            && Math.abs(0.5f - noise(x, y - i * 999, 1, 1, (30 + i * 4)))
                            > 0.37f * thresh) {
                        ore = dest;
                    }
                });
    ...
    public void oreAround(Block ore, Block wall, int radius, float scl, float thresh) {
        for (Tile tile : tiles) {
            ...
            if (tile.block() == Blocks.air
                    ...
                    && noise(x, y + ore.id * 999, scl, 1f) > thresh) {
                boolean found = false;

                outer:
                for (int dx = x - radius; dx <= x + radius; dx++) {
                    for (int dy = y - radius; dy <= y + radius; dy++) {
                        if (Mathf.within(dx, dy, x, y, radius + 0.001f)
                                && tiles.in(dx, dy)
                                && tiles.get(dx, dy).block() == wall) {
                            found = true;
                            break outer;
                        }
                    }
    ...
    public void terrain(Block dst, float scl, float mag, float cmag) {
        pass(
                (x, y) -> {
                    double rocks =
                            noise(x, y, 5, 0.5, scl) * mag
                                    + Mathf.dst((float) x / width, (float) y / height, 0.5f, 0.5f)
                                    * cmag;
    ...
    public void noise(
            Block floor, Block block, int octaves, float falloff, float scl, float threshold) {
        pass(
                (x, y) -> {
    ...
    public void tech(Block floor1, Block floor2, Block wall) {
        ...
        pass(
                (x, y) -> {
                    ...
                    if (noise(sclx, scly, 0.2f, 1f) > 0.63f
                            && noise(sclx, scly + 999, 200f, 1f) > 0.6f
                            && (mx == 0 || my == 0 || mx == secSize - 1 || my == secSize - 1)) {
        ...
        tiles.each(
                (x, y) -> {
                    ...
                    float cx = x + noise(x - 155f, y - 200f, scl, mag) - mag / 2f,
                            cy = y + noise(x + 155f, y + 155f, scl, mag) - mag / 2f;
                    Tile other =
                            tiles.getn(
                                    Mathf.clamp((int) cx, 0, tiles.width - 1),
                                    Mathf.clamp((int) cy, 0, tiles.height - 1));
                    ...
                    (x, y) -> {
                        ...
                        for (int cx = -cradius; cx <= cradius; cx++) {
                            for (int cy = -cradius; cy <= cradius; cy++) {
                                if ((cx == 0 && cy == 0) || !Mathf.within(cx, cy, cradius))
                                    continue;
    ...
    protected abstract float noise(
    ...
    public Seq<Tile> pathfind(
            int startX, int startY, int endX, int endY, TileHueristic th, DistanceHeuristic dh) {
        return Astar.pathfind(
                ...
                th,
                dh,
                tile -> world.getDarkness(tile.x, tile.y) <= 1f);
    }
