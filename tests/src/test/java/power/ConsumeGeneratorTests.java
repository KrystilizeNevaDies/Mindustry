...
/**
 * This class tests generators which can process items, liquids or both. All tests are run with a
 * fixed delta of 0.5 so delta considerations can be tested as well. Any expected power amount
 * (produced, consumed, buffered) should be affected by fakeDelta but status should not!
 */
public class ConsumeGeneratorTests extends PowerTestFixture {
    ...
    public void createGenerator(InputType inputType) {
        ...
        ConsumeGenerator generator =
                new ConsumeGenerator("fakegen" + System.nanoTime()) {
                    {
    ...
    DynamicTest[] generatorWorksProperlyWithLiquidInput() {
        ...
        // Execute all tests for the case where only liquids are accepted and for the case where
        // liquids
        // and items are accepted (but supply only liquids)
        InputType[] inputTypesToBeTested =
                new InputType[]{
                        InputType.liquids,
                        // InputType.any
                };
                        ...
                        dynamicTest(
                                ...
                                () ->
                                        simulateLiquidConsumption(
                                                ...
                                                inputType,
                                                Liquids.oil,
                                                maximumLiquidUsage * 1.0f,
                                                "Sufficient oil provided")));
                tests.add(
                        dynamicTest(
                                "04-delta" + d,
                                () ->
    ...
    void simulateLiquidConsumption(
            float delta,
            InputType inputType,
            Liquid liquid,
            float availableLiquidAmount,
            String parameterDescription) {
        Time.setDeltaProvider(() -> delta);

        float expectedConsumptionPerTick =
                Math.min(maximumLiquidUsage * Time.delta, availableLiquidAmount);
        ...
        float expectedRemainingLiquidAmount =
                Math.max(0.0f, availableLiquidAmount - expectedConsumptionPerTick);

        createGenerator(inputType);
        assertTrue(
                build.acceptLiquid(null, liquid),
                inputType
                        + " | "
                        + parameterDescription
                        + ": Liquids which will be declined by the generator don't need to be tested - The code won't be called for those cases.");
        ...
        // Perform an update on the generator once - This should use up any resource up to the
        // maximum
        // liquid usage
        build.update();
        ...
        assertEquals(
                expectedEfficiency,
                build.efficiency(),
                inputType + " | " + parameterDescription + ": Base input efficiency mismatch.");
        assertEquals(
                expectedRemainingLiquidAmount,
                build.liquids.get(liquid),
                inputType + " | " + parameterDescription + ": Remaining liquid amount mismatch.");
        assertEquals(
                expectedOutputEfficiency,
                build.productionEfficiency,
                inputType
                        + " | "
                        + parameterDescription
                        + ": Output production efficiency mismatch.");
    }
    ...
    DynamicTest[] generatorWorksProperlyWithItemInput() {
        ...
        // Execute all tests for the case where only items are accepted and for the case where
        // liquids
        // and items are accepted (but supply only items)
        InputType[] inputTypesToBeTested =
                new InputType[]{
                        InputType.items,
                        // InputType.any
                    ...
                    dynamicTest(
                            ...
                            () ->
                                    simulateItemConsumption(
                                            inputType, Items.coal, 0, "No items provided")));
            tests.add(
                    dynamicTest(
                            "02",
                            () ->
                                    simulateItemConsumption(
                                            inputType, Items.coal, 1, "Sufficient coal provided")));
            tests.add(
                    dynamicTest(
                            "03",
                            () ->
                                    simulateItemConsumption(
                                            inputType, Items.coal, 10, "Excess coal provided")));
            tests.add(
                    dynamicTest(
                            "04",
                            () ->
                                    simulateItemConsumption(
                                            ...
                                            Items.blastCompound,
                                            1,
                                            "Blast compound provided")));
            tests.add(
                    dynamicTest(
                            "05",
                            () ->
                                    simulateItemConsumption(
                                            inputType, Items.sporePod, 1, "Biomatter provided")));
            tests.add(
                    dynamicTest(
                            "06",
                            () ->
                                    simulateItemConsumption(
                                            inputType, Items.pyratite, 1, "Pyratite provided")));
        }
        return tests.toArray(DynamicTest.class);
    }

    void simulateItemConsumption(
            InputType inputType, Item item, int amount, String parameterDescription) {
        float expectedEfficiency = amount > 0 ? item.flammability : 0f;
        int expectedRemainingItemAmount = Math.max(0, amount - 1);

        createGenerator(inputType);
        assertTrue(
                build.acceptItem(null, item),
                inputType
                        + " | "
                        + parameterDescription
                        + ": Items which will be declined by the generator don't need to be tested - The code won't be called for those cases.");

        if (amount > 0) {
            build.items.add(item, amount);
        }

        // Perform an update on the generator once - This should use up one or zero items -
        // dependent on
        // if the item is accepted and available or not.
        build.update();

        assertEquals(
                expectedRemainingItemAmount,
                build.items.get(item),
                inputType + " | " + parameterDescription + ": Remaining item amount mismatch.");
        assertEquals(
                expectedEfficiency,
                build.productionEfficiency,
                inputType + " | " + parameterDescription + ": Efficiency mismatch.");
    }
    ...
    void testItemDuration(InputType inputType) {
        ...
        while ((currentDuration += Time.delta) <= fakeItemDuration) {
            build.update();
            assertEquals(
                    expectedEfficiency, build.productionEfficiency, "Duration: " + currentDuration);
        }
        build.update();
