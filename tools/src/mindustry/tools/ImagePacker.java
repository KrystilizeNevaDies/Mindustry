...
public class ImagePacker {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // don't write to the file unless I'm packing, because logic IDs rarely change and I don't
        // want
        // merge conflicts from PRs
        if (!OS.username.equals("anuke")) return;

        // format: ([content type (byte)] [content count (short)] (repeat [name (string)])) until
        // EOF
        ...
        for (ContentType t : GlobalVars.lookableContent) {
            lookupCont.addAll(
                    Vars.content
                            .<UnlockableContent>getBy(t)
                            .select(UnlockableContent::logicVisible));
        }
        ...
        for (UnlockableContent c : lookupCont) {
            ...
            if (!registered[ctype].containsKey(c)) {
                int nextId = 0;
                // find next ID - this is O(N) but content counts are so low that I don't really
                // care
        ...
        try (DataOutputStream out = new DataOutputStream(logicidfile.write(false, 2048))) {
            for (ContentType t : GlobalVars.lookableContent) {
                Seq<UnlockableContent> all =
                        idToContent[t.ordinal()]
                                .values()
                                .toArray()
                                .sort(u -> registered[t.ordinal()].get(u));
