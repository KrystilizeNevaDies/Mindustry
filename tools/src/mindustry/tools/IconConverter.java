...
public class IconConverter {
    ...
    public static void main(String[] __) {
        ...
        for (Fi img : list) {
            if (img.extension().equals("png")) {
                Fi dst =
                        new Fi("fontgen/icons")
                                .child(img.nameWithoutExtension().replace("icon-", "") + ".svg");
        ...
        Seq<String> args =
                Seq.with(
                        ...
                        "--batch-process",
                        "--actions",
                        "select-all; path-union; fit-canvas-to-selection; export-overwrite; export-do");
        args.addAll(files.map(Fi::absolutePath));

        Fi.get("fontgen/extra")
                .findAll()
                .each(f -> f.copyTo(Fi.get("fontgen/icons").child(f.name())));
    ...
    void convert(Pixmap pixmap, Fi output) {
        ...
        float xscl = 1f, yscl = 1f; // resolution / (float)pixmap.getWidth(), yscl = resolution /
        // (float)pixmap.getHeight();
        float scl = xscl;

        width = pixmap.width;
        height = pixmap.height;

        out.append("<svg width=\"")
                .append(pixmap.width)
                .append("\" height=\"")
