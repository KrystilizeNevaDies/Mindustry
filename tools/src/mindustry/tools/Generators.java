...
public class Generators {
    ...
    private static float fluid(boolean gas, double x, double y, float frame) {
        ...
        if (gas) {
            ...
            float interpolated =
                    Mathf.lerp(
                            (float) gasFrame(x, y, curFrame),
                            (float) gasFrame(x, y, nextFrame),
                            progress);
            ...
            float interpolated =
                    (float)
                            liquidFrame(
                                    rx, ry, 2); // Mathf.lerp((float)liquidFrame(rx, ry, curFrame),
            // (float)liquidFrame(rx, ry,
            // nextFrame), progress);
    ...
    private static double gasFrame(double x, double y, int frame) {
        ...
        return (Simplex.rawTiled(x, y, ox, oy, s, s, scale)
                + Simplex.rawTiled(x, y, ox, oy, s, s, scale / 1.5) * second)
                / (1.0 + second);
    }
    ...
    public static void run() {
        ...
        generate(
                ...
                () -> {
                    ...
                    for (int i = 0; i < frames; i++) {
                        ...
                        float shinex = size / 2f + shinelen / Mathf.sqrt2,
                                shiney = size / 2f - shinelen / Mathf.sqrt2;
                        ...
                        pixmap.each(
                                (x, y) -> {
                                    float dst = Mathf.dst(x, y, size / 2f, size / 2f);
                                    if (Math.abs(dst - radius) <= stroke
                                            || Mathf.within(x, y, shinex, shiney, shinerad)) {
                                        pixmap.set(x, y, Color.white);
                                    }
                                });
                                                ...
                                                if (m != 0) {
                                                    ...
                                                    if (m == 3) {
                                                        ...
                                                        for (int rad = 9; rad < 64; rad += 7) {
                                                            for (int cx = Math.max(x - rad, 0);
                                                                 cx
                                                                         <= Math.min(
                                                                         x + rad,
                                                                         size - 1);
                                                                 cx++) {
                                                                for (int cy = Math.max(y - rad, 0);
                                                                     cy
                                                                             <= Math.min(
                                                                             y + rad,
                                                                             size - 1);
                    ...
                    for (int size = 1; size <= BlockRenderer.maxCrackSize; size++) {
                        ...
                        for (int i = 0; i < steps; i++) {
                            ...
                            for (int x = 0; x < dim; x++) {
                                for (int y = 0; y < dim; y++) {
                                    float dst =
                                            Mathf.dst((float) x / dim, (float) y / dim, 0.5f, 0.5f)
                                                    * 2f;
                                    if (dst < 1.2f
                                            && Ridged.noise2d(1, x, y, 3, 1f / 40f)
                                            - dst * (1f - fract)
                                            > 0.16f) {
                            ...
                            for (int x = 0; x < output.width; x++) {
                                for (int y = 0; y < output.height; y++) {
                                    ...
                                    output.setRaw(
                                            x,
                                            y,
                                            whites >= clears ? Color.whiteRgba : Color.clearRgba);
                                }
                            }
        ...
        generate(
                ...
                () -> {
                    ...
                    for (Block block : content.blocks()) {
                        if (block.isAir()
                                || block instanceof ConstructBlock
                                || block instanceof OreBlock
                                || block instanceof LegacyBlock) continue;
                        ...
                        for (TextureRegion region : block.makeIconRegions()) {
                            GenRegion gen = (GenRegion) region;
                            save(
                                    get(region).outline(block.outlineColor, block.outlineRadius),
                                    gen.name + "-outline");
                        }
                        ...
                        try {
                            ...
                            if (block.outlineIcon) {
                                ...
                                Pixmap out =
                                        last =
                                                base.outline(
                                                        block.outlineColor, block.outlineRadius);
                            ...
                            for (TextureRegion region : regions) {
                                ...
                                if (region == block.teamRegions[Team.sharded.id]
                                        && shardTeamTop != null) {
                            ...
                            if (block.buildVisibility != BuildVisibility.hidden) {
                                saveScaled(
                                        image,
                                        block.name + "-icon-logic",
                                        Math.min(32 * 3, image.width));
                            }
                            saveScaled(
                                    image,
                                    "../ui/block-" + block.name + "-ui",
                                    Math.min(image.width, maxUiIcon));
        ...
        generate(
                "item-icons",
                () -> {
                    for (UnlockableContent item :
                            Seq.<UnlockableContent>withArrays(
                                    content.items(), content.liquids(), content.statusEffects())) {
                        if (item instanceof StatusEffect
                                && !has(item.getContentType().name() + "-" + item.name)) {
                            continue;
                        }
                            ...
                            base.each(
                                    (x, y) ->
                                            tint.setRaw(
                                                    ...
                                                    y,
                                                    Color.muli(
                                                            tint.getRaw(x, y), stat.color.rgba())));
                        ...
                        save(
                                base,
                                "../ui/" + item.getContentType().name() + "-" + item.name + "-ui");
                    }
                });
                ...
                () -> {
                    for (Team team : Team.all) {
                        if (has("team-" + team.name)) {
                            int rgba =
                                    team == Team.derelict
                                            ? Color.valueOf("b7b8c9").rgba()
                                            : team.color.rgba();
                            Pixmap base = get("team-" + team.name);
                            base.each(
                                    (x, y) ->
                                            base.setRaw(x, y, Color.muli(base.getRaw(x, y), rgba)));
                                ...
                                .each(
                                        type -> {
                                            ...
                                            try {
                                                ...
                                                Func<Pixmap, Pixmap> outline =
                                                        i -> i.outline(type.outlineColor, 3);
                                                Cons<TextureRegion> outliner =
                                                        t -> {
                                                            if (t != null && t.found()) {
                                                ...
                                                for (Weapon weapon : weapons) {
                                                    if (outlined.add(weapon.name)
                                                            && has(weapon.name)) {
                                                        // only non-top weapons need separate
                                                        // outline sprites (this is mostly
                                                        // just mechs)
                                                        if (!weapon.top
                                                                || weapon.parts.contains(
                                                                p -> p.under)) {
                                                            save(
                                                                    outline.get(get(weapon.name)),
                                                                    weapon.name + "-outline");
                                                        } else {
                                                            // replace weapon with outlined version,
                                                            // no use keeping standard
                                                            // around
                                                ...
                                                Pixmap image =
                                                        type.segments > 0
                                                                ? get(type.segmentRegions[0])
                                                                : outline.get(
                                                                get(type.previewRegion));

                                                Func<Weapon, Pixmap> weaponRegion =
                                                        weapon ->
                                                                Core.atlas.has(
                                                                        weapon.name
                                                                                + "-preview")
                                                                        ...
                                                                        weapon.name
                                                                                + "-preview")
                                                                        : get(weapon.region);
                                                Cons2<Weapon, Pixmap> drawWeapon =
                                                        (weapon, pixmap) ->
                                                                image.draw(
                                                                        weapon.flipSprite
                                                                                ? pixmap.flipX()
                                                ...
                                                // draw each extra segment on top before it is saved
                                                // as outline
                                                if (sample instanceof Crawlc) {
                                                    for (int i = 0; i < type.segments; i++) {
                                                        // replace(type.segmentRegions[i],
                                                        // outline.get(get(type.segmentRegions[i])));
                                                        save(
                                                                outline.get(
                                                                        get(
                                                                                type.segmentRegions[
                                                                                        i])),
                                                                type.name + "-segment-outline" + i);

                                                        if (i > 0) {
                                                            drawCenter(
                                                                    image,
                                                                    get(type.segmentRegions[i]));
                                                        }
                                                    }
                                                    save(image, type.name);
                                                }

                                                // outline is currently never needed, although it
                                                ...
                                                for (Weapon weapon :
                                                        weapons.select(w -> w.layerOffset < 0)) {
                                                    drawWeapon.get(
                                                            weapon,
                                                            outline.get(weaponRegion.get(weapon)));
                                                    anyUnder = true;
                                                }
                                                ...
                                                if (anyUnder) {
                                                    image.draw(
                                                            outline.get(get(type.previewRegion)),
                                                            true);
                                                }
                                                ...
                                                if (sample instanceof Tankc) {
                                                    Pixmap treads =
                                                            outline.get(get(type.treadRegion));
                                                    image.draw(
                                                            treads,
                                                            image.width / 2 - treads.width / 2,
                                                            image.height / 2 - treads.height / 2,
                                                            true);
                                                ...
                                                if (type.drawCell) {
                                                    ...
                                                    // replace with 0xffd37fff : 0xdca463ff for
                                                    // sharded colors?
                                                    cell.replace(
                                                            in ->
                                                                    in == 0xffffffff
                                                    ...
                                                    image.draw(
                                                            cell,
                                                            image.width / 2 - cell.width / 2,
                                                            image.height / 2 - cell.height / 2,
                                                            true);
                                                }
                                                ...
                                                for (Weapon weapon : weapons) {
                                                    ...
                                                    Pixmap wepReg =
                                                            weapon.top ? outline.get(reg) : reg;
                                                ...
                                                // TODO I can save a LOT of space by not creating a
                                                // full icon.
                                                ...
                                                float offsetRange =
                                                        Math.max(image.width, image.height) * 0.15f;
                                                Vec2 offset =
                                                        new Vec2(1, 1)
                                                                .rotate(rand.random(360f))
                                                                .setLength(
                                                                        rand.random(0, offsetRange))
                                                                .add(
                                                                        image.width / 2f,
                                                                        image.height / 2f);
                                                ...
                                                for (int i = 0; i < wrecks.length; i++) {
                                                    wrecks[i] =
                                                            new Pixmap(image.width, image.height);
                                                }
                            ...
                            .<OreBlock>each(
                                    ...
                                    ore -> {
                                        ...
                                        for (int i = 0; i < ore.variants; i++) {
                                            ...
                                            save(
                                                    image,
                                                    "../blocks/environment/" + ore.name + (i + 1));
        ...
        generate(
                ...
                () -> {
                    content.blocks()
                            .<Floor>each(
                                    b -> b instanceof Floor && !(b instanceof OverlayFloor),
                                    floor -> {
                                        if (has(floor.name + "-edge")
                                                || floor.blendGroup != floor) {
                                            return;
                                        }
        ...
        private double noise(float angle) {
            return Math.pow(
                    Simplex.noise2d(
                            ...
                            pers,
                            1 / scale,
                            Angles.trnsx(angle, size / 2f) + size / 2f,
                            Angles.trnsy(angle, size / 2f) + size / 2f),
                    pow);
        }
    }
}
