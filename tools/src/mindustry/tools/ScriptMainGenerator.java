...
public class ScriptMainGenerator {
    ...
    public static void main(String[] args) throws Exception {
        ...
        classes.removeAll(
                type ->
                        type.isSynthetic()
                                ...
                                || blacklist.contains(
                                s -> type.getName().startsWith(base + "." + s + "."))
                                || nameBlacklist.contains(type.getSimpleName()));
        ...
        for (Class type : classes) {
            if (used.contains(type.getPackage().getName())
                    || nopackage.contains(s -> type.getName().startsWith(s))) continue;
            result.append("importPackage(Packages.")
                ...
                classes.select(
                                c ->
                                        Modifier.isPublic(c.getModifiers())
                                                && packages.contains(
                                                c.getCanonicalName()::startsWith))
                        .add(Block.class); // special case

        for (Class<?> c : mapped) {
            cdef.append("        classes.put(\"")
        ...
        new Fi("core/src/mindustry/mod/ClassMap.java")
                .writeString(classTemplate.replace("$CLASSES$", cdef.toString()));
    ...
    public static Seq<Class> getClasses(String packageName) throws Exception {
        ...
        for (URL resource :
                Collections.list(classLoader.getResources(packageName.replace('.', '/')))) {
